...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/jar4/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Jar4 Re-Auth" -c "After getting key, run: ssh $SERVER '~/jar4/scripts/mobile-reauth.sh'" --id clawd-auth
        ;;
    *)
