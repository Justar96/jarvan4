...
# Build and bundle Jar4 into a minimal .app we can open.
# Outputs to dist/Jar4.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Jar4.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Jar4"
BUNDLE_ID="${BUNDLE_ID:-com.jar4.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/jar4/jar4/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Jar4/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :Jar4BuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :Jar4GitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Jar4"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Jar4"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Jar4"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Jar4" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Jar4/Resources/Jar4.icns" "$APP_ROOT/Contents/Resources/Jar4.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Jar4/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying Jar4Kit resources"
MOLTBOTKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/Jar4Kit_Jar4Kit.bundle"
if [ -d "$MOLTBOTKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/Jar4Kit_Jar4Kit.bundle"
  cp -R "$MOLTBOTKIT_BUNDLE" "$APP_ROOT/Contents/Resources/Jar4Kit_Jar4Kit.bundle"
else
  echo "WARN: Jar4Kit resource bundle not found at $MOLTBOTKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Jar4"
killall -q Jar4 2>/dev/null || true
