...
#
# Env:
#   DMG_VOLUME_NAME        default: CFBundleName (or "Jar4")
...
APP_NAME=$(/usr/libexec/PlistBuddy -c "Print CFBundleName" "$APP_PATH/Contents/Info.plist" 2>/dev/null || echo "Jar4")
...
DMG_TEMP="$(mktemp -d /tmp/jar4-dmg.XXXXXX)"
...
if [[ "${SKIP_DMG_STYLE:-0}" != "1" ]]; then
  ...
  ICON_SRC="$ROOT_DIR/apps/macos/Sources/Jar4/Resources/Jar4.icns"
  if [[ -f "$ICON_SRC" ]]; then
...
hdiutil resize -limits "$DMG_RW_PATH" >/tmp/jar4-dmg-limits.txt 2>/dev/null || true
MIN_SECTORS="$(tail -n 1 /tmp/jar4-dmg-limits.txt 2>/dev/null | awk '{print $1}')"
rm -f /tmp/jar4-dmg-limits.txt
