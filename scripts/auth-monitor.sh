...
#
# Suggested cron: */30 * * * * /home/admin/jar4/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Jar4 notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., jar4-alerts)
...
STATE_FILE="$HOME/.jar4/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Jar4 if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use jar4
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Jar4 to $NOTIFY_PHONE..."
            jar4 send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Jar4 is down. Run: ssh l36 '~/jar4/scripts/mobile-reauth.sh'" "urgent"
    exit 1
