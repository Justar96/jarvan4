#!/usr/bin/env bash
# Reset Jar4 like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_BUNDLE="${CLAWDBOT_APP_BUNDLE:-}"
APP_PROCESS_PATTERN="Jar4.app/Contents/MacOS/Jar4"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/debug/Jar4"
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/Jar4"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/Jar4"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/com.jar4.mac.plist"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/jar4-restart-${LOCK_KEY}"
LOCK_PID_FILE="${LOCK_DIR}/pid"
WAIT_FOR_LOCK=0
LOG_PATH="${CLAWDBOT_RESTART_LOG:-/tmp/jar4-restart.log}"
NO_SIGN=0
SIGN=0
AUTO_DETECT_SIGNING=1
GATEWAY_WAIT_SECONDS="${CLAWDBOT_GATEWAY_WAIT_SECONDS:-0}"
LAUNCHAGENT_DISABLE_MARKER="${HOME}/.jar4/disable-launchagent"
ATTACH_ONLY=1
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log "  rm ~/.jar4/disable-launchagent"
      log ""
...
stop_launch_agent() {
  launchctl bootout gui/"$UID"/com.jar4.mac 2>/dev/null || true
}

# 1) Kill all running instances first.
log "==> Killing existing Jar4 instances"
kill_all_jar4
stop_launch_agent
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product Jar4"
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/Jar4.app" ]]; then
    APP_BUNDLE="/Applications/Jar4.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/Jar4.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/Jar4.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/Jar4.app missing Sparkle after packaging"
    fi
    return 0
  fi

  fail "App bundle not found. Set CLAWDBOT_APP_BUNDLE to your installed Jar4.app"
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const raw = fs.readFileSync(path.join(process.env.HOME, ".jar4", "jar4.json"), "utf8");
...
if pgrep -f "${APP_PROCESS_PATTERN}" >/dev/null 2>&1; then
  log "OK: Jar4 is running."
else
  fail "App exited immediately. Check ${LOG_PATH} or Console.app (User Reports)."
fi

if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "show gateway launch agent args (unsigned)" bash -lc "/usr/bin/plutil -p '${HOME}/Library/LaunchAgents/com.jar4.gateway.plist' | head -n 40 || true"
fi
