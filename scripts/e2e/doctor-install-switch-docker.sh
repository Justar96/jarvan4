...
IMAGE_NAME="jar4-doctor-install-switch-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/jar4-bin:$PATH"
  mkdir -p /tmp/jar4-bin

  cat > /tmp/jar4-bin/systemctl <<"SYSTEMCTL"
#!/usr/bin/env bash
set -euo pipefail
...
SYSTEMCTL
  chmod +x /tmp/jar4-bin/systemctl

  cat > /tmp/jar4-bin/loginctl <<"LOGINCTL"
#!/usr/bin/env bash
set -euo pipefail
...
LOGINCTL
  ...
  npm_bin="/tmp/npm-prefix/bin/jar4"
  npm_entry="/tmp/npm-prefix/lib/node_modules/jar4/dist/entry.js"
  ...
  run_flow() {
    ...
    echo "== Flow: $name =="
    home_dir=$(mktemp -d "/tmp/jar4-switch-${name}.XXXXXX")
    export HOME="$home_dir"
    export USER="testuser"

    eval "$install_cmd"

    unit_path="$HOME/.config/systemd/user/jar4-gateway.service"
    if [ ! -f "$unit_path" ]; then
