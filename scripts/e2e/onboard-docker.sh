...
TRASH
  ...
  run_wizard_cmd() {
    ...
    input_fifo="$(mktemp -u "/tmp/jar4-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/jar4-onboard-${case_name}.log"
  ...
  make_home() {
    mktemp -d "/tmp/jar4-e2e-$1.XXXXXX"
  }
  ...
  run_case_local_basic() {
    ...
    config_path="$HOME/.jar4/jar4.json"
    sessions_dir="$HOME/.jar4/agents/main/sessions"
