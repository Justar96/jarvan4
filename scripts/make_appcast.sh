...
ZIP=${1:?"Usage: $0 Jar4-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/jar4/jar4/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^Jar4-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/jar4/jar4/releases/download/v${VERSION}/"}
