#!/bin/bash
# Setup Jar4 Auth Management System
...
echo "  2. Subscribe to a topic (e.g., 'jar4-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Jar4 message: Send warning via Jar4 itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/jar4-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/jar4-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/jar4-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now jar4-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status jar4-auth-monitor.timer"
echo ""
echo "Quick commands:"
