...
# Checks both Claude Code and Jar4 auth status

set -euo pipefail

CLAUDE_CREDS="$HOME/.claude/.credentials.json"
CLAWDBOT_AUTH="$HOME/.jar4/agents/main/agent/auth-profiles.json"
...
fetch_models_status_json() {
    jar4 models status --json 2>/dev/null || true
}
...
if [ "$OUTPUT_MODE" = "json" ]; then
    ...
    jar4_status=$(check_jar4_auth 2>/dev/null || true)

    claude_expires=0
    jar4_expires=0
    if [ "$USE_JSON" -eq 1 ]; then
        claude_expires=$(json_expires_for_claude_cli)
        jar4_expires=$(json_expires_for_anthropic_any)
    else
        claude_expires=$(jq -r '.claudeAiOauth.expiresAt // 0' "$CLAUDE_CREDS" 2>/dev/null || echo "0")
        jar4_expires=$(jq -r '.profiles["anthropic:default"].expires // 0' "$CLAWDBOT_AUTH" 2>/dev/null || echo "0")
    fi
    ...
    jq -n \
        ...
        '{
            ...
            jar4: {status: $bs, expires_at_ms: ($be | tonumber)},
...
if [ "$OUTPUT_MODE" = "simple" ]; then
    ...
    if [[ "$claude_status" == EXPIRED* ]] || [[ "$claude_status" == MISSING* ]]; then
        echo "CLAUDE_EXPIRED"
        exit 1
    elif [[ "$jar4_status" == EXPIRED* ]] || [[ "$jar4_status" == MISSING* ]]; then
        echo "CLAWDBOT_EXPIRED"
        exit 1
    elif [[ "$claude_status" == EXPIRING* ]]; then
        echo "CLAUDE_EXPIRING"
        exit 2
    elif [[ "$jar4_status" == EXPIRING* ]]; then
...
echo ""
echo "Jar4 Auth (~/.jar4/agents/main/agent/auth-profiles.json):"
if [ "$USE_JSON" -eq 1 ]; then
...
if [ "$expires" -le 0 ] && [ "$api_keys" -gt 0 ]; then
    ...
    echo -e "  Status: ${RED}NOT FOUND${NC}"
    echo "  Note: Run 'jar4 doctor --yes' to sync from Claude Code"
else
    now_ms=$(( $(date +%s) * 1000 ))
...
echo ""
echo "=== Service Status ==="
if systemctl --user is-active jar4 >/dev/null 2>&1; then
    echo -e "Jar4 service: ${GREEN}running${NC}"
else
    echo -e "Jar4 service: ${RED}NOT running${NC}"
fi
