...
show_usage() {
    cat << EOF
clawlog - Jar4 Logging Utility

USAGE:
    clawlog [OPTIONS]

DESCRIPTION:
    View Jar4 logs with full details (bypasses Apple's privacy redaction).
    Requires sudo access configured for /usr/bin/log command.

LOG FLOW ARCHITECTURE:
    Jar4 logs flow through the macOS unified log (subsystem: com.jar4).
