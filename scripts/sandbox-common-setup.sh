...
BASE_IMAGE="${BASE_IMAGE:-jar4-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-jar4-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=jar4.sandbox=1)
NOTE
