...
Remote Gateway:
```bash
jar4 tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `jar4 tui --deliver`
...
- Check the Gateway logs: `jar4 logs --follow`.
- Confirm the agent can run: `jar4 status` and `jar4 models status`.
- If you expect messages in a chat channel, enable delivery (`/deliver on` or `--deliver`).
- `--history-limit <n>`: History entries to load (default 200)

## Troubleshooting
- `disconnected`: ensure the Gateway is running and your `--url/--token/--password` are correct.
- No agents in picker: check `jar4 agents list` and your routing config.
