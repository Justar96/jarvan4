...
read_when:
  ...
  - Deciding: standalone CLI vs deep Jar4 integration
...
Recommendation: **deep integration in Jar4**, but keep a separable core library.

### Why integrate into Jar4?
- Jar4 already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `jar4 memory recall "…" --k 25 --since 30d`
  - `jar4 memory reflect --since 7d`
