...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `jar4 sandbox explain`.
...
Legacy `agent.*` configs are migrated by `jar4 doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ```exec
   docker ps --filter "label=jar4.sandbox=1"
   ```
...
4. **Monitor logs:**
   ```exec
   tail -f "${JAR4_STATE_DIR:-$HOME/.jar4}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
