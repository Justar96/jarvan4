---
summary: "OpenProse: .prose workflows, slash commands, and state in Jar4"
read_when:
...
OpenProse is a portable, markdown-first workflow format for orchestrating AI sessions. In Jar4 it ships as a plugin that installs an OpenProse skill pack plus a `/prose` slash command. Programs live in `.prose` files and can spawn multiple sub-agents with explicit control flow.
...
```bash
jar4 plugins enable open-prose
```

Restart the Gateway after enabling the plugin.

Dev/local checkout: `jar4 plugins install ./extensions/open-prose`

Related docs: [Plugins](/plugin), [Plugin manifest](/plugins/manifest), [Skills](/tools/skills).

## Slash command

OpenProse registers `/prose` as a user-invocable skill command. It routes to the OpenProse VM instructions and uses Jar4 tools under the hood.
...
## Jar4 runtime mapping

OpenProse programs map to Jar4 primitives:

| OpenProse concept | Jar4 tool |
| --- | --- |
...
Treat `.prose` files like code. Review before running. Use Jar4 tool allowlists and approval gates to control side effects.
