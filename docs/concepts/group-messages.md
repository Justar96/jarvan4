...
Add a `groupChat` block to `~/.jar4/jar4.json` so display-name pings work even when WhatsApp strips the visual `@` in the text body:
...
Notes:
- The regexes are case-insensitive; they cover a display-name ping like `@jar4` and the raw number with or without `+`/spaces.
...
## How to use
1) Add your WhatsApp account (the one running Jar4) to the group.
2) Say `@jar4 …` (or include the number). Only allowlisted senders can trigger it unless you set `groupPolicy: "open"`.
...
- Session store entries will appear as `agent:<agentId>:whatsapp:group:<jid>` in the session store (`~/.jar4/agents/<agentId>/sessions/sessions.json` by default); a missing entry just means the group hasn’t triggered a run yet.
