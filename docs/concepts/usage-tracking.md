...
- `/usage cost` in chats: local cost summary aggregated from Jar4 session logs.
- CLI: `jar4 status --usage` prints a full per-provider breakdown.
- CLI: `jar4 channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
