...
- **Session store** (chat history + routing state) under `~/.jar4/agents/<agentId>/sessions`.
...
available from `~/.jar4/skills`. See [Skills: per-agent vs shared](/tools/skills#per-agent-vs-shared-skills).
...
- State dir: `~/.jar4` (or `JAR4_STATE_DIR`)
- Workspace: `~/clawd` (or `~/clawd-<agentId>`)
- Agent dir: `~/.jar4/agents/<agentId>/agent` (or `agents.list[].agentDir`)
- Sessions: `~/.jar4/agents/<agentId>/sessions`

### Single-agent mode (default)

If you do nothing, Jar4 runs a single agent:

- `agentId` defaults to **`main`**.
- Sessions are keyed as `agent:main:<mainKey>`.
- Workspace defaults to `~/clawd` (or `~/clawd-<profile>` when `JAR4_PROFILE` is set).
- State defaults to `~/.jar4/agents/main/agent`.
...
```js
{
    ...
    list: [
      {
        ...
        agentDir: "~/.jar4/agents/home/agent",
      },
      {
        id: "work",
        name: "Work",
        workspace: "~/clawd-work",
        agentDir: "~/.jar4/agents/work/agent",
      },
    ],
  },
  ...
  channels: {
    whatsapp: {
      accounts: {
        personal: {
          // Optional override. Default: ~/.jar4/credentials/whatsapp/personal
          // authDir: "~/.jar4/credentials/whatsapp/personal",
        },
        biz: {
