---
summary: "All configuration options for ~/.jar4/jar4.json with examples"
read_when:
  - Adding or modifying config fields
---
# Configuration 🔧

Jar4 reads an optional **JSON5** config from `~/.jar4/jar4.json` (comments + trailing commas allowed).

If the file is missing, Jar4 uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/clawd`). You usually only need a config to:
...
Jar4 only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:
- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `jar4 doctor`, `jar4 logs`, `jar4 health`, `jar4 status`, `jar4 service`, `jar4 help`).
- Run `jar4 doctor` to see the exact issues.
- Run `jar4 doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `jar4 config set`. Keep a backup of `~/.jar4/jar4.json`.
...
```bash
jar4 gateway call config.get --params '{}' # capture payload.hash
jar4 gateway call config.apply --params '{
...
```bash
jar4 gateway call config.get --params '{}' # capture payload.hash
jar4 gateway call config.patch --params '{
...
{ "$include": "/etc/jar4/base.json5" } // absolute
...
```json5
// ~/.jar4/clients/mueller/agents.json5
[
...
```json5
// ~/.jar4/clients/mueller/broadcast.json5
{
...
Jar4 reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, Jar4 runs your login shell and imports only the missing expected keys (never overrides).
...
Env var equivalent:
- `JAR4_LOAD_SHELL_ENV=1`
- `JAR4_SHELL_ENV_TIMEOUT_MS=15000`
...
Jar4 stores **per-agent** auth profiles (OAuth + API keys) in:
- `<agentDir>/auth-profiles.json` (default: `~/.jar4/agents/<agentId>/agent/auth-profiles.json`)

See also: [/concepts/oauth](/concepts/oauth)

Legacy OAuth imports:
- `~/.jar4/credentials/oauth.json` (or `$JAR4_STATE_DIR/credentials/oauth.json`)

The embedded Pi agent maintains a runtime cache at:
- `<agentDir>/auth.json` (managed automatically; don’t edit manually)

Legacy agent dir (pre multi-agent):
- `~/.jar4/agent/*` (migrated by `jar4 doctor` into `~/.jar4/agents/<defaultAgentId>/agent/*`)

Overrides:
- OAuth dir (legacy import only): `JAR4_OAUTH_DIR`
- Agent dir (default agent root override): `JAR4_AGENT_DIR` (preferred), `PI_CODING_AGENT_DIR` (legacy)

On first use, Jar4 imports `oauth.json` entries into `auth-profiles.json`.
...
If set, Jar4 derives defaults (only when you haven’t set them explicitly):
...
- Default log file: `/tmp/jar4/jar4-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/jar4/jar4.log`.
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // Optional override. Default: ~/.jar4/credentials/whatsapp/biz
          // authDir: "~/.jar4/credentials/whatsapp/biz",
        }
      }
    }
  }
...
- The legacy single-account Baileys auth dir is migrated by `jar4 doctor` into `whatsapp/default`.
...
```json5
{
  ...
  agents: {
    list: [
      { id: "main", groupChat: { mentionPatterns: ["@clawd", "jar4", "clawd"] } }
    ]
  }
}
```
...
- `agents.list[]`: per-agent overrides.
  ...
  - `agentDir`: default `~/.jar4/agents/<agentId>/agent`.
...
- `commands.config: true` enables `/config` (reads/writes `jar4.json`).
...
Jar4 starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
...
Jar4 starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `jar4 plugins install @jar4/mattermost` (or `./extensions/mattermost` from a git checkout).
...
Jar4 starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
Jar4 spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, Jar4
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/jar4" } }
}
```
...
When a file exceeds this limit, Jar4 logs a warning and injects a truncated
...
message envelopes). If unset, Jar4 uses the host timezone at runtime.
...
`[jar4]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[jar4]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[jar4]"`, Jar4 will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, Jar4 generates audio
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.jar4/settings/tts.json",
      elevenlabs: {
...
Jar4 also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, Jar4 currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `jar4 configure --section web`, or use `BRAVE_API_KEY` env var)
...
- `group:nodes`: `nodes`
- `group:jar4`: all built-in Jar4 tools (excludes provider plugins)
...
- agent workspace access: `workspaceAccess: "none"` (default)
  - `"none"`: use a per-scope sandbox workspace under `~/.jar4/sandboxes`
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.jar4/sandboxes",
        docker: {
          image: "jar4-sandbox:bookworm-slim",
          containerPrefix: "jar4-sbx-",
          workdir: "/workspace",
          readOnlyRoot: true,
        ...
        browser: {
          enabled: false,
          image: "jar4-sandbox-browser:bookworm-slim",
          containerPrefix: "jar4-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
Jar4 uses the **pi-coding-agent** model catalog. You can add custom providers
(LiteLLM, local OpenAI-compatible servers, Anthropic proxies, etc.) by writing
`~/.jar4/agents/<agentId>/agent/models.json` or by defining the same schema inside your
Jar4 config under `models.providers`.
Provider-by-provider overview + examples: [/concepts/model-providers](/concepts/model-providers).

When `models.providers` is present, Jar4 writes/merges a `models.json` into
`~/.jar4/agents/<agentId>/agent/` on startup:
...
OpenCode Zen is a multi-model gateway with per-model endpoints. Jar4 uses
...
- Shortcut: `jar4 onboard --auth-choice opencode-zen`.
...
Shortcut: `jar4 onboard --auth-choice zai-api-key`.
...
Notes:
- Set `MOONSHOT_API_KEY` in the environment or use `jar4 onboard --auth-choice moonshot-api-key`.
...
Notes:
- Set `KIMICODE_API_KEY` in the environment or use `jar4 onboard --auth-choice kimi-code-api-key`.
...
Notes:
- Set `SYNTHETIC_API_KEY` or use `jar4 onboard --auth-choice synthetic-api-key`.
...
Notes:
- Set `MINIMAX_API_KEY` environment variable or use `jar4 onboard --auth-choice minimax-api`.
...
- Override the agent config root with `JAR4_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
  if you want `models.json` stored elsewhere (default: `~/.jar4/agents/main/agent`).
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.jar4/agents/<agentId>/sessions/sessions.json
    // You can override with {agentId} templating:
    store: "~/.jar4/agents/{agentId}/sessions/sessions.json",
...
- `resetByType`: per-session overrides for `dm`, `group`, and `thread`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, Jar4 stays in idle-only mode for backward compatibility.
...
overrides. Applies to **bundled** skills and `~/.jar4/skills` (workspace skills
...
from `~/.jar4/extensions`, `<workspace>/.jar4/extensions`, plus any
...
Jar4 can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for clawd and expose a small loopback control service.
...
- Note: the control server is started by the running gateway (Jar4.app menubar, or `jar4 gateway`).
...
```json5
{
  gateway: {
    ...
    bind: "loopback",
    // controlUi: { enabled: true, basePath: "/jar4" }
...
- When a connection comes from one of these IPs, Jar4 uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:
- `jar4 gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
- `gateway.port` controls the single multiplexed port used for WebSocket + HTTP (control UI, hooks, A2UI).
- OpenAI Chat Completions endpoint: **disabled by default**; enable with `gateway.http.endpoints.chatCompletions.enabled: true`.
- Precedence: `--port` > `JAR4_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- `gateway.auth.password` can be set here, or via `JAR4_GATEWAY_PASSWORD` (recommended).
- `gateway.auth.allowTailscale` allows Tailscale Serve identity headers
  (`tailscale-user-login`) to satisfy auth when the request arrives on loopback
  with `x-forwarded-for`, `x-forwarded-proto`, and `x-forwarded-host`. Jar4
...
macOS app behavior:
- Jar4.app watches `~/.jar4/jar4.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.jar4/jar4.json` (or `JAR4_CONFIG_PATH`) and applies changes automatically.
...
Files watched:
- `~/.jar4/jar4.json` (or `JAR4_CONFIG_PATH`)
...
- `JAR4_STATE_DIR` (sessions/creds)
- `agents.defaults.workspace` (memories)
- `gateway.port` (unique per instance)

Convenience flags (CLI):
- `jar4 --dev …` → uses `~/.jar4-dev` + shifts ports from base `19001`
- `jar4 --profile <name> …` → uses `~/.jar4-<name>` (port via config/env/flags)
...
Example:
```bash
JAR4_CONFIG_PATH=~/.jar4/a.json \
JAR4_STATE_DIR=~/.jar4-a \
jar4 gateway --port 19001
```
...
Gmail helper config (used by `jar4 webhooks gmail setup` / `run`):

```json5
{
  hooks: {
    gmail: {
      account: "jar4@gmail.com",
...
- Set `JAR4_SKIP_GMAIL_WATCHER=1` to disable the auto-start (for manual runs).
...
Note: when `tailscale.mode` is on, Jar4 defaults `serve.path` to `/` so
...
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__jar4/ws`
- auto-creates a starter `index.html` when the directory is empty (so you see something immediately)
- also serves A2UI at `/__jar4__/a2ui/` and is advertised to nodes as `canvasHostUrl`
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.jar4/bridge/tls/bridge-{cert,key}.pem when omitted.
      // certPath: "~/.jar4/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.jar4/bridge/tls/bridge-key.pem"
    }
  }
}
```
...
Controls LAN mDNS discovery broadcasts (`_jar4-gw._tcp`).
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_jar4-bridge._tcp` under `~/.jar4/dns/` using the standard discovery domain `jar4.internal.`

To make iOS/Android discover across networks (Vienna ⇄ London), pair this with:
- a DNS server on the gateway host serving `jar4.internal.` (CoreDNS is recommended)
- Tailscale **split DNS** so clients resolve `jar4.internal` via that server
