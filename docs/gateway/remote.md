...
- Use the macOS app’s **Remote over SSH** mode (Settings → General → “Jar4 runs”).
...
With the tunnel up:
- `jar4 health` and `jar4 status --deep` now reach the remote gateway via `ws://127.0.0.1:18789`.
- `jar4 gateway {status,health,send,agent,call}` can also target the forwarded URL via `--url` when needed.

Note: replace `18789` with your configured `gateway.port` (or `--port`/`JAR4_GATEWAY_PORT`).
