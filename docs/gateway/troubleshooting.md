---
summary: "Quick troubleshooting guide for common Jar4 failures"
read_when:
  - Investigating runtime issues or failures
---
# Troubleshooting 🔧

When Jar4 misbehaves, here's how to fix it.
...
|---|---|---|
| `jar4 status` | Local summary: OS + update, gateway reachability/mode, service, agents/sessions, provider config state | First check, quick overview |
| `jar4 status --all` | Full local diagnosis (read-only, pasteable, safe-ish) incl. log tail | When you need to share a debug report |
| `jar4 status --deep` | Runs gateway health checks (incl. provider probes; requires reachable gateway) | When “configured” doesn’t mean “working” |
| `jar4 gateway probe` | Gateway discovery + reachability (local + remote targets) | When you suspect you’re probing the wrong gateway |
| `jar4 channels status --probe` | Asks the running gateway for channel status (and optionally probes) | When gateway is reachable but channels misbehave |
| `jar4 gateway status` | Supervisor state (launchd/systemd/schtasks), runtime PID/exit, last gateway error | When the service “looks loaded” but nothing runs |
| `jar4 logs --follow` | Live logs (best signal for runtime issues) | When you need the actual failure reason |

**Sharing output:** prefer `jar4 status --all` (it redacts tokens). If you paste `jar4 status`, consider setting `JAR4_SHOW_SECRETS=0` first (token previews).
...
- Or paste a setup-token on the **gateway host**:
  ```bash
  jar4 models auth setup-token --provider anthropic
  ```
...
```bash
jar4 models auth paste-token --provider anthropic
jar4 models status
```
...
**Logs:**
- Preferred: `jar4 logs --follow`
- File logs (always): `/tmp/jar4/jar4-YYYY-MM-DD.log` (or your configured `logging.file`)
- macOS LaunchAgent (if installed): `$JAR4_STATE_DIR/logs/gateway.log` and `gateway.err.log`
- Linux systemd (if installed): `journalctl --user -u jar4-gateway[-<profile>].service -n 200 --no-pager`
...
- Or set it directly:
  ```bash
  jar4 config set gateway.mode local
  ```

**If you meant to run a remote Gateway instead:**
- Set a remote URL and keep `gateway.mode=remote`:
  ```bash
  jar4 config set gateway.mode remote
  jar4 config set gateway.remote.url "wss://gateway.example.com"
  ```

**Ad-hoc/dev only:** pass `--allow-unconfigured` to start the gateway without
`gateway.mode=local`.

**No config file yet?** Run `jar4 setup` to create a starter config, then rerun
the gateway.
...
variables like `DISPLAY` should live in `~/.jar4/.env` (loaded early by the
gateway).
...
service was installed with Bun or a version-managed Node path, run `jar4 doctor`
...
- then run `jar4 sandbox recreate --agent <id>` (or `--all`)
...
**Check:**
- `gateway.mode` must be `local` for `jar4 gateway` and the service.
- If you set `gateway.mode=remote`, the **CLI defaults** to a remote URL. The service can still be running locally, but your CLI may be probing the wrong place. Use `jar4 gateway status` to see the service’s resolved port + probe target (or pass `--url`).
- `jar4 gateway status` and `jar4 doctor` surface the **last gateway error** from logs when the service looks running but the port is closed.
- Non-loopback binds (`lan`/`tailnet`/`custom`, or `auto` when loopback is unavailable) require auth:
  `gateway.auth.token` (or `JAR4_GATEWAY_TOKEN`).
- `gateway.remote.token` is for remote CLI calls only; it does **not** enable local auth.
- `gateway.token` is ignored; use `gateway.auth.token`.

**If `jar4 gateway status` shows a config mismatch**
...
- Fix: rerun `jar4 gateway install --force` from the same `--profile` / `JAR4_STATE_DIR` you want the service to use.

**If `jar4 gateway status` reports service config issues**
- The supervisor config (launchd/systemd/schtasks) is missing current defaults.
- Fix: run `jar4 doctor` to update it (or `jar4 gateway install --force` for a full rewrite).
...
- Fix: set `gateway.auth.mode` + `gateway.auth.token` (or export `JAR4_GATEWAY_TOKEN`) and restart the service.

**If `jar4 gateway status` says `bind=tailnet` but no tailnet interface was found**
...
If you upgraded from older installs, you might still have `~/jar4` on disk.
...
Symptoms: `pwd` or file tools show `~/.jar4/sandboxes/...` even though you
...
Jar4 intentionally rejects **older/insecure models** (especially those more
...
- If you’re unsure which models are available, run `jar4 models list` or
  `jar4 models scan` and choose a supported one.
...
grep -n "agents\\|groupChat\\|mentionPatterns\\|channels\\.whatsapp\\.groups\\|channels\\.telegram\\.groups\\|channels\\.imessage\\.groups\\|channels\\.discord\\.guilds" \
  "${JAR4_CONFIG_PATH:-$HOME/.jar4/jar4.json}"
```
...
tail -f "$(ls -t /tmp/jar4/jar4-*.log | head -1)" | grep "blocked\\|skip\\|unauthorized"
```
...
```bash
jar4 logs --follow | grep "pairing request"
```
...
```bash
ls -la ~/.jar4/agents/<agentId>/sessions/
```
...
jar4 logs --limit 200 | grep "connection\\|disconnect\\|logout"
```
...
```bash
jar4 gateway --verbose
```

If you’re logged out / unlinked:

```bash
jar4 channels logout
trash "${JAR4_STATE_DIR:-$HOME/.jar4}/credentials" # if logout can't cleanly remove everything
jar4 channels login --verbose       # re-scan QR
```
...
```bash
grep "media\\|fetch\\|download" "$(ls -t /tmp/jar4/jar4-*.log | head -1)" | tail -20
```

### High Memory Usage

Jar4 keeps conversation history in memory.
...
Jar4 now refuses to start when the config contains unknown keys, malformed values, or invalid types.
...
Notes:
- `jar4 doctor` reports every invalid entry.
- `jar4 doctor --fix` applies migrations/repairs and rewrites the config.
- Diagnostic commands like `jar4 logs`, `jar4 health`, `jar4 status`, `jar4 gateway status`, and `jar4 gateway probe` still run even if the config is invalid.

### “All models failed” — what should I check first?

- **Credentials** present for the provider(s) being tried (auth profiles + env vars).
- **Model routing**: confirm `agents.defaults.model.primary` and fallbacks are models you can access.
- **Gateway logs** in `/tmp/jar4/…` for the exact provider error.
- **Model status**: use `/model status` (chat) or `jar4 models status` (CLI).
...
5) Run `jar4 channels status --probe` for audit hints.
...
endpoint accepts a strict subset of JSON Schema. Jar4 scrubs/normalizes tool
...
```bash
tccutil reset All com.jar4.mac.debug
```

**Fix 2: Force New Bundle ID**
If resetting doesn't work, change the `BUNDLE_ID` in [`scripts/package-mac-app.sh`](https://github.com/jar4/jar4/blob/main/scripts/package-mac-app.sh) (e.g., add a `.test` suffix) and rebuild. This forces macOS to treat it as a new app.
...
jar4 gateway status
jar4 gateway stop
# Or: launchctl bootout gui/$UID/com.jar4.gateway (replace with com.jar4.<profile> if needed)
```
...
Ensure the global `jar4` CLI is installed and matches the app version:
```bash
jar4 --version
...
#   ${JAR4_CONFIG_PATH:-$HOME/.jar4/jar4.json} -> { logging: { level: "trace" } }
#
# Then run verbose commands to mirror debug output to stdout:
jar4 gateway --verbose
jar4 channels login --verbose
```
...
| Log | Location |
|-----|----------|
| Gateway file logs (structured) | `/tmp/jar4/jar4-YYYY-MM-DD.log` (or `logging.file`) |
| Gateway service logs (supervisor) | macOS: `$JAR4_STATE_DIR/logs/gateway.log` + `gateway.err.log` (default: `~/.jar4/logs/...`; profiles use `~/.jar4-<profile>/logs/...`)<br />Linux: `journalctl --user -u jar4-gateway[-<profile>].service -n 200 --no-pager`<br />Windows: `schtasks /Query /TN "Jar4 Gateway (<profile>)" /V /FO LIST` |
| Session files | `$JAR4_STATE_DIR/agents/<agentId>/sessions/` |
| Media cache | `$JAR4_STATE_DIR/media/` |
| Credentials | `$JAR4_STATE_DIR/credentials/` |
...
trash "${JAR4_STATE_DIR:-$HOME/.jar4}"
jar4 channels login         # re-pair WhatsApp
jar4 gateway restart           # or: jar4 gateway
```
...
1. Check logs first: `/tmp/jar4/` (default: `jar4-YYYY-MM-DD.log`, or your configured `logging.file`)
