...
- During heartbeat runs, Jar4 treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `jar4 sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Jar4 skips the heartbeat run entirely (no model call).
...
```bash
jar4 system event --text "Check for urgent follow-ups" --mode now
```
