...
`jar4 doctor` is the repair + migration tool for Jar4. It fixes stale
...
- Extra workspace dir detection (`~/jar4`).
...
you to run `jar4 doctor`.

Doctor will:
- Explain which legacy keys were found.
- Show the migration it applied.
- Rewrite `~/.jar4/jar4.json` with the updated schema.
...
- Sessions store + transcripts:
  - from `~/.jar4/sessions/` to `~/.jar4/agents/<agentId>/sessions/`
- Agent dir:
  - from `~/.jar4/agent/` to `~/.jar4/agents/<agentId>/agent/`
- WhatsApp auth state (Baileys):
  - from legacy `~/.jar4/credentials/*.json` (except `oauth.json`)
  - to `~/.jar4/credentials/whatsapp/<accountId>/...` (default account id: `default`)

These migrations are best-effort and idempotent; doctor will emit warnings when
it leaves any legacy folders behind as backups. The Gateway/CLI also auto-migrates
the legacy sessions + agent dir on startup so history/auth/models land in the
per-agent path without a manual doctor run. WhatsApp auth is intentionally only
migrated via `jar4 doctor`.
...
- **Multiple state dirs**: warns when multiple `~/.jar4` folders exist across
  home directories or when `JAR4_STATE_DIR` points elsewhere (history can
  split between installs).
- **Remote mode reminder**: if `gateway.mode=remote`, doctor reminds you to run
  it on the remote host (the state lives there).
- **Config file permissions**: warns if `~/.jar4/jar4.json` is
...
offers to remove them and install the Jar4 service using the current gateway
port. It can also scan for extra gateway-like services and print cleanup hints.
Profile-named Jar4 gateway services are considered first-class and are not
flagged as "extra."
...
generate a token. Use `jar4 doctor --generate-gateway-token` to force token
...
Notes:
- `jar4 doctor` prompts before rewriting supervisor config.
- `jar4 doctor --yes` accepts the default repair prompts.
- `jar4 doctor --repair` applies recommended fixes without prompts.
- `jar4 doctor --repair --force` overwrites custom supervisor configs.
- You can always force a full rewrite via `jar4 gateway install --force`.
