...
- Replaces the legacy `gateway` command. CLI entry point: `jar4 gateway`.
...
jar4 gateway --port 18789 --verbose
# if the port is busy, terminate listeners then start:
jar4 gateway --force
# dev loop (auto-reload on TS changes):
pnpm gateway:watch
```
- Config hot reload watches `~/.jar4/jar4.json` (or `JAR4_CONFIG_PATH`).
...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__jar4__/canvas/` from `~/clawd/canvas`. Disable with `canvasHost.enabled=false` or `JAR4_SKIP_CANVAS_HOST=1`.
...
- Gateway auth is required by default: set `gateway.auth.token` (or `JAR4_GATEWAY_TOKEN`) or `gateway.auth.password`. Clients must send `connect.params.auth.token/password` unless using Tailscale Serve identity.
- The wizard now generates a token by default, even on loopback.
- Port precedence: `--port` > `JAR4_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- macOS: `com.jar4.<profile>`
- Linux: `jar4-gateway-<profile>.service`
- Windows: `Jar4 Gateway (<profile>)`

Install metadata is embedded in the service config:
- `JAR4_SERVICE_MARKER=jar4`
- `JAR4_SERVICE_KIND=gateway`
- `JAR4_SERVICE_VERSION=<version>`
...
```bash
jar4 --dev setup
jar4 --dev gateway --allow-unconfigured
...
- `JAR4_STATE_DIR=~/.jar4-dev`
- `JAR4_CONFIG_PATH=~/.jar4-dev/jar4.json`
- `JAR4_GATEWAY_PORT=19001` (Gateway WS + HTTP)
...
- Base port = `gateway.port` (or `JAR4_GATEWAY_PORT` / `--port`)
- browser control service port = base + 2 (loopback only)
- `canvasHost.port = base + 4` (or `JAR4_CANVAS_HOST_PORT` / config override)
- Browser profile CDP ports auto-allocate from `browser.controlPort + 9 .. + 108` (persisted per profile).

Checklist per instance:
- unique `gateway.port`
- unique `JAR4_CONFIG_PATH`
- unique `JAR4_STATE_DIR`
- unique `agents.defaults.workspace`
- separate WhatsApp numbers (if using WA)

Service install per profile:
```bash
jar4 --profile main gateway install
jar4 --profile rescue gateway install
```

Example:
```bash
JAR4_CONFIG_PATH=~/.jar4/a.json JAR4_STATE_DIR=~/.jar4-a jar4 gateway --port 19001
JAR4_CONFIG_PATH=~/.jar4/b.json JAR4_STATE_DIR=~/.jar4-b jar4 gateway --port 19002
```
...
## Methods (initial set)
- `health` — full health snapshot (same shape as `jar4 health --json`).
...
- Use launchd to keep the service alive:
  - Program: path to `jar4`
  - Arguments: `gateway`
  - KeepAlive: true
  - StandardOut/Err: file paths or `syslog`
- On failure, launchd restarts; fatal misconfig should keep exiting so the operator notices.
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - `jar4 gateway install` writes `~/Library/LaunchAgents/com.jar4.gateway.plist`
    (or `com.jar4.<profile>.plist`).
  - `jar4 doctor` audits the LaunchAgent config and can update it to current defaults.
...
- If other gateway-like services are detected, the CLI warns unless they are Jar4 profile services.
  ...
  - Cleanup: `jar4 gateway uninstall` (current service) and `jar4 doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use `jar4 gateway install --force` to reinstall (profile/env/path changes).

Bundled mac app:
- Jar4.app can bundle a Node-based gateway relay and install a per-user LaunchAgent labeled
  `com.jar4.gateway` (or `com.jar4.<profile>`).
- To stop it cleanly, use `jar4 gateway stop` (or `launchctl bootout gui/$UID/com.jar4.gateway`).
- To restart, use `jar4 gateway restart` (or `launchctl kickstart -k gui/$UID/com.jar4.gateway`).
  - `launchctl` only works if the LaunchAgent is installed; otherwise use `jar4 gateway install` first.
  - Replace the label with `com.jar4.<profile>` when running a named profile.

## Supervision (systemd user unit)
Jar4 installs a **systemd user service** by default on Linux/WSL2. We
...
`jar4 gateway install` writes the user unit. `jar4 doctor` audits the
...
Restart=always
RestartSec=5
Environment=JAR4_GATEWAY_TOKEN=
...
Create `/etc/systemd/system/jar4-gateway[-<profile>].service` (copy the unit above,
switch `WantedBy=multi-user.target`, set `User=` + `WorkingDirectory=`), then:
```
sudo systemctl daemon-reload
sudo systemctl enable --now jar4-gateway[-<profile>].service
```
...
## CLI helpers
- `jar4 gateway health|status` — request health/status over the Gateway WS.
- `jar4 message send --target <num> --message "hi" [--media ...]` — send via Gateway (idempotent for WhatsApp).
- `jar4 agent --message "hi" --to <num>` — run an agent turn (waits for final by default).
- `jar4 gateway call <method> --params '{"k":"v"}'` — raw method invoker for debugging.
- `jar4 gateway stop|restart` — stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance
- Retire uses of `jar4 gateway` and the legacy TCP control port.
