...
## Goals
- Talk to Jar4 via Discord DMs or guild channels.
...
3. Configure Jar4 with `channels.discord.token` (or `DISCORD_BOT_TOKEN` as a fallback).
...
7. Direct chats: secure by default via `channels.discord.dm.policy` (default: `"pairing"`). Unknown senders get a pairing code (expires after 1 hour); approve via `jar4 pairing approve discord <code>`.
...
This is the “Discord Developer Portal” setup for running Jar4 in a server (guild) channel like `#help`.
...
### 2) Enable the gateway intents Jar4 needs
...
Discord uses numeric ids everywhere; Jar4 config prefers ids.
...
### Troubleshooting
- First: run `jar4 doctor` and `jar4 channels status --probe` (actionable warnings + quick audits).
...
- On startup, Jar4 resolves channel/user names in allowlists to IDs (when the bot can search members)
  and logs the mapping; unresolved entries are kept as typed.

Native command notes:
- The registered commands mirror Jar4’s chat commands.
...
- Slash commands may still be visible in Discord UI to users who aren’t allowlisted; Jar4 enforces allowlists on execution and replies “not authorized”.
...
- If the bot is stuck or rate limited, restart the gateway (`jar4 gateway --force`) after confirming no other processes own the Discord session.
