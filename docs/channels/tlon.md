...
Tlon is a decentralized messenger built on Urbit. Jar4 connects to your Urbit ship and can
...
```bash
jar4 plugins install @jar4/tlon
```

Local checkout (when running from a git repo):

```bash
jar4 plugins install ./extensions/tlon
```
...
Use these with `jar4 message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Jar4 replies in-thread.
