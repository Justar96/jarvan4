...
read_when:
  - You want Jar4 to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Jar4 to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`jar4 onboard`) and `jar4 channels add` list optional channel plugins.
...
```bash
jar4 plugins install @jar4/nostr
```

Use a local checkout (dev workflows):

```bash
jar4 plugins install --link <path-to-jar4>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "jar4@example.com",
        "lud16": "jar4@example.com"
      }
    }
  }
}
