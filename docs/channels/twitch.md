...
read_when:
  - Setting up Twitch chat integration for Jar4
---
# Twitch (plugin)

Twitch chat support via IRC connection. Jar4 connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
jar4 plugins install @jar4/twitch
```

Local checkout (when running from a git repo):

```bash
jar4 plugins install ./extensions/twitch
```
...
4) Configure the token:
   - Env: `JAR4_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "jar4",              // Bot's Twitch account
      accessToken: "oauth:abc123...",    // OAuth Access Token (or use JAR4_TWITCH_ACCESS_TOKEN env var)
...
```bash
JAR4_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
