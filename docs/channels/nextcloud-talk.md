...
```bash
jar4 plugins install @jar4/nextcloud-talk
```

Local checkout (when running from a git repo):
```bash
jar4 plugins install ./extensions/nextcloud-talk
```
...
2) On your Nextcloud server, create a bot:
   ```bash
   ./occ talk:bot:install "Jar4" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `jar4 pairing list nextcloud-talk`
  - `jar4 pairing approve nextcloud-talk <CODE>`
