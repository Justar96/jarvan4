...
Matrix is an open, decentralized messaging protocol. Jar4 connects as a Matrix **user**
...
```bash
jar4 plugins install @jar4/matrix
```

Local checkout (when running from a git repo):

```bash
jar4 plugins install ./extensions/matrix
```
...
1) Install the Matrix plugin:
   - From npm: `jar4 plugins install @jar4/matrix`
   - From a local checkout: `jar4 plugins install ./extensions/matrix`
...
3) Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: Jar4 calls the same
     login endpoint, stores the access token in `~/.jar4/credentials/matrix/credentials.json`,
...
- On first connection, Jar4 requests device verification from your other sessions.
...
`~/.jar4/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `jar4 pairing list matrix`
  - `jar4 pairing approve matrix <CODE>`
...
- On startup, Jar4 resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
