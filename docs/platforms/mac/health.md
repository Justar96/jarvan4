...
## How the probe works
- App runs `jar4 health --json` via `ShellExecutor` every ~60s and on demand. The probe loads creds and reports status without sending messages.
- Cache the last good snapshot and the last error separately to avoid flicker; show the timestamp of each.

## When in doubt
- You can still use the CLI flow in [Gateway health](/gateway/health) (`jar4 status`, `jar4 status --deep`, `jar4 health --json`) and tail `/tmp/jar4/jar4-*.log` for `web-heartbeat` / `web-reconnect`.
