---
summary: "Jar4 on DigitalOcean (simple paid VPS option)"
read_when:
  - Setting up Jar4 on DigitalOcean
  - Looking for cheap VPS hosting for Jar4
---

# Jar4 on DigitalOcean

## Goal

Run a persistent Jar4 Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
# Check service
systemctl --user status jar4-gateway.service

# View logs
journalctl --user -u jar4-gateway.service -f
```
...
```bash
jar4 config set gateway.bind tailnet
jar4 gateway restart
```
...
journalctl -u jar4 --no-pager -n 50
```
