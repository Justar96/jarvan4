---
summary: "Jar4 on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Jar4 on a Raspberry Pi
  - Running Jar4 on ARM devices
  - Building a cheap always-on personal AI
---

# Jar4 on Raspberry Pi

## Goal

Run a persistent, always-on Jar4 Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most Jar4 features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled jar4

# Enable if not
sudo systemctl enable jar4

# Start on boot
sudo systemctl start jar4
```
...
```bash
# Check logs
journalctl -u jar4 --no-pager -n 100

# Common fix: rebuild
cd ~/jar4  # if using hackable install
npm run build
sudo systemctl restart jar4
```
