---
summary: "Run Jar4 Gateway on exe.dev (VM + HTTPS proxy) for remote access"
read_when:
...
Goal: Jar4 Gateway running on an exe.dev VM, reachable from your laptop via:
...
1) Create VM → install Node 22 → install Jar4  
2) Run `jar4 onboard --install-daemon`  
...
Tip: keep this VM **stateful**. Jar4 stores state under `~/.jar4/` and `~/clawd/`.
...
```bash
ssh -N -L 18789:127.0.0.1:18789 jar4.exe.xyz
```
...
```bash
export JAR4_GATEWAY_TOKEN="$(openssl rand -hex 32)"
jar4 gateway --bind lan --port 8080 --token "$JAR4_GATEWAY_TOKEN"
```

For service runs, persist it in `~/.jar4/jar4.json`:
...
On Linux, Jar4 uses a systemd **user** service. After `--install-daemon`, verify:

```bash
systemctl --user status jar4-gateway[-<profile>].service
```
