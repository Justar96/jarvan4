...
Jar4 features that can generate provider usage or paid API calls.
...
- `jar4 status --usage` and `jar4 channels list` show provider **usage windows**
...
Jar4 can pick up credentials from:
...
### 8) Model scan / probe
`jar4 models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
