...
This document explains how Jar4 manages sessions end-to-end:
...
Jar4 is designed around a single **Gateway process** that owns session state.
...
Jar4 persists sessions in two layers:
...
- Transcripts: `~/.jar4/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

Jar4 resolves these via `src/config/sessions.ts`.
...
Jar4 intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (Jar4 consumes the events, but Pi decides when to compact).
...
Jar4 also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, Jar4 bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, Jar4 leaves it alone.
...
Jar4 supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:
- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- Jar4 strips/suppresses this in the delivery layer.

As of `2026.1.10`, Jar4 also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
Jar4 uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but Jar4’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `jar4 status`.
