...
Broadcast groups are evaluated after channel allowlists and group activation rules. In WhatsApp groups, this means broadcasts happen when Jar4 would normally reply (for example: on mention, depending on your group settings).
...
**Result:** When Jar4 would reply in this chat, it will run all three agents.
...
**Debug:**
```bash
tail -f ~/.jar4/logs/gateway.log | grep broadcast
```
