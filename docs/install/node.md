...
read_when:
  - "You installed Jar4 but `jar4` is “command not found”"
...
Jar4’s runtime baseline is **Node 22+**.

If you can run `npm install -g jar4@latest` but later see `jar4: command not found`, it’s almost always a **PATH** issue: the directory where npm puts global binaries isn’t on your shell’s PATH.
