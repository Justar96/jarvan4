...
Delivery:
- Publish as `@jar4/plugin-sdk` (or export from core under `jar4/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)
Scope: everything that touches core runtime behavior.
Accessed via `Jar4PluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: Jar4Config, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: Jar4Config): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: Jar4Config, agentId?: string): RegExp[];
      matchesMentionPatterns(text: string, regexes: RegExp[]): boolean;
    };
    groups: {
      resolveGroupPolicy(cfg: Jar4Config, channel: string, accountId: string, groupId: string): {
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: Jar4Config, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: Jar4Config): string;
  };
};
```
...
### Phase 0: scaffolding
- Introduce `@jar4/plugin-sdk`.
- Add `api.runtime` to `Jar4PluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `jar4Runtime: ">=2026.2.0"`).
