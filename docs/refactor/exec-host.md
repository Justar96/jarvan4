...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.jar4/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.jar4/node.json` (node id + pairing token).
...
### IPC
- Unix socket at `~/.jar4/exec-approvals.sock` (0600).
