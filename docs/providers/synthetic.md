---
summary: "Use Synthetic's Anthropic-compatible API in Jar4"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. Jar4 registers it as the
...
```bash
jar4 onboard --auth-choice synthetic-api-key
```
...
Note: Jar4's Anthropic client appends `/v1` to the base URL, so use
