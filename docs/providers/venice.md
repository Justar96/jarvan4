---
summary: "Use Venice AI privacy-focused models in Jar4"
read_when:
  - You want privacy-focused inference in Jar4
...
```bash
jar4 onboard --auth-choice venice-api-key
```
...
```bash
jar4 chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, Jar4 shows all available Venice models. Pick based on your needs:
...
```bash
jar4 models set venice/claude-opus-45
jar4 models set venice/llama-3.3-70b
```

List all available models:

```bash
jar4 models list | grep venice
```

## Configure via `jar4 configure`

1. Run `jar4 configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Jar4 automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
jar4 chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
jar4 chat --model venice/claude-opus-45

# Use uncensored model
jar4 chat --model venice/venice-uncensored

# Use vision model with image
jar4 chat --model venice/qwen3-vl-235b-a22b

# Use coding model
jar4 chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `jar4 models list` to see currently available models. Some models may be temporarily offline.
