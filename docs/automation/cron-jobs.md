...
- Jobs persist under `~/.jar4/cron/` so restarts don’t lose schedules.
...
## Storage & history
- Job store: `~/.jar4/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.jar4/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
jar4 cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
jar4 cron edit <jobId> --agent ops
jar4 cron edit <jobId> --clear-agent
```
```
...
```bash
jar4 system event --mode now --text "Next heartbeat: check battery."
```

## Gateway API surface
- `cron.list`, `cron.status`, `cron.add`, `cron.update`, `cron.remove`
- `cron.run` (force or due), `cron.runs`
For immediate system events without a job, use [`jar4 system event`](/cli/system).

## Troubleshooting

### “Nothing runs”
- Check cron is enabled: `cron.enabled` and `JAR4_SKIP_CRON`.
