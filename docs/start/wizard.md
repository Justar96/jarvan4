...
The onboarding wizard is the **recommended** way to set up Jar4 on macOS,
...
`jar4 dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
...
(`web_fetch` works without a key). Easiest path: `jar4 configure --section web`
...
1) **Existing config detection**
   - If `~/.jar4/jar4.json` exists, choose **Keep / Modify / Reset**.
   - Re-running the wizard does **not** wipe anything unless you explicitly choose **Reset**
     (or pass `--reset`).
   - If the config is invalid or contains legacy keys, the wizard stops and asks
     you to run `jar4 doctor` before continuing.
...
2) **Model/Auth**
   ...
   - **OpenAI API key**: uses `OPENAI_API_KEY` if present or prompts for a key, then saves it to `~/.jar4/.env` so launchd can read it.
  ...
  - OAuth credentials live in `~/.jar4/credentials/oauth.json`; auth profiles live in `~/.jar4/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth).
...
5) **Channels**
  ...
  - DM security: default is pairing. First DM sends a code; approve via `jar4 pairing approve <channel> <code>` or use allowlists.
...
7) **Health check**
   - Starts the Gateway (if needed) and runs `jar4 health`.
   - Tip: `jar4 status --deep` adds gateway health probes to status output (requires a reachable gateway).
...
Use `jar4 agents add <name>` to create a separate agent with its own workspace,
...
- Stores it under `~/.jar4/tools/signal-cli/<version>/`.
...
Typical fields in `~/.jar4/jar4.json`:
...
`jar4 agents add` writes `agents.list[]` and optional `bindings`.

WhatsApp credentials go under `~/.jar4/credentials/whatsapp/<accountId>/`.
Sessions are stored under `~/.jar4/agents/<agentId>/sessions/`.
