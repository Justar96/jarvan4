---
summary: "First-run onboarding flow for Jar4 (macOS app)"
read_when:
...
- Writes credentials to `~/.jar4/credentials/oauth.json`
...
The app can install the global `jar4` CLI via npm/pnpm so terminal
...
On the first agent run, Jar4 bootstraps a workspace (default `~/clawd`):
...
```bash
jar4 webhooks gmail setup --account you@gmail.com
```
