...
Fastest chat: open the Control UI (no channel setup needed). Run `jar4 dashboard`
and chat in the browser, or open `http://127.0.0.1:18789/` on the gateway host.
Docs: [Dashboard](/web/dashboard) and [Control UI](/web/control-ui).

Recommended path: use the **CLI onboarding wizard** (`jar4 onboard`). It sets up:
...
- **Recommended:** Brave Search API key for web search. Easiest path:
  `jar4 configure --section web` (stores `tools.web.search.apiKey`).
...
- OAuth credentials (legacy import): `~/.jar4/credentials/oauth.json`
...
If you’re hacking on Jar4 itself, run from source:
...
If `jar4 health` shows “no auth configured”, go back to the wizard and set OAuth/key auth — the agent won’t be able to respond without it.

Tip: `jar4 status --all` is the best pasteable, read-only debug report.
Health probes: `jar4 health` (or `jar4 status --deep`) asks the running gateway for a health snapshot.
