---
summary: "Chrome extension: let Jar4 drive your existing Chrome tab"
read_when:
...
The Jar4 Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate clawd-managed Chrome profile.
...
Jar4 then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the Jar4 release (npm package) as static files. There is no separate “build” step.

After upgrading Jar4:
- Re-run `jar4 browser extension install` to refresh the installed files under your Jar4 state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

Jar4 ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want Jar4 to control.
...
- `ON`: attached; Jar4 can drive that tab.
...
Debugging: `jar4 sandbox explain`
...
`jar4 browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `jar4 browser extension install` first to copy the extension to a stable location under your Jar4 state directory.
