---
summary: "Agent tool surface for Jar4 (browser, canvas, nodes, message, cron) replacing legacy `jar4-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `jar4-*` skills
---

# Tools (Jar4)

Jar4 exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `jar4-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `jar4.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, Jar4 logs a warning and ignores the allowlist so core tools stay available.
...
- `host=node` can target a macOS companion app or a headless node host (`jar4 node run`).
...
Notes:
- Requires a Brave API key (recommended: `jar4 configure --section web`, or set `BRAVE_API_KEY`).
...
- Quick smoke: `jar4 nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
Core actions:
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `jar4 gateway` restart in-place)
