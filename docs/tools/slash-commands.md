...
- `commands.config` (default `false`) enables `/config` (reads/writes `jar4.json`).
...
- For full provider usage breakdown, use `jar4 status --usage`.
- `/allowlist add|remove` requires `commands.config=true` and honors channel `configWrites`.
- `/usage` controls the per-response usage footer; `/usage cost` prints a local cost summary from Jar4 session logs.
...
Notes:
- Overrides apply immediately to new config reads, but do **not** write to `jar4.json`.
- Use `/debug reset` to clear all overrides and return to the on-disk config.

## Config updates

`/config` writes to your on-disk config (`jar4.json`). Owner-only. Disabled by default; enable with `commands.config: true`.
...
/config set messages.responsePrefix="[jar4]"
