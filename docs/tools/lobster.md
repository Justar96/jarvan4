---
title: Lobster
summary: "Typed workflow runtime for Jar4 with resumable approval gates."
description: Typed workflow runtime for Jar4 — composable pipelines with approval gates.
read_when:
...
Lobster is a workflow shell that lets Jar4 run multi-step tool sequences as a single, deterministic operation with explicit approval checkpoints.
...
- **One call instead of many**: Jar4 runs one Lobster tool call and gets a structured result.
...
Jar4 launches the local `lobster` CLI in **tool mode** and parses a JSON envelope from stdout.
...
Lobster can run YAML/JSON workflow files with `name`, `args`, `steps`, `env`, `condition`, and `approval` fields. In Jar4 tool calls, set `pipeline` to the file path.
...
Install the Lobster CLI on the **same host** that runs the Jar4 Gateway (see the [Lobster repo](https://github.com/jar4/lobster)), and ensure `lobster` is on `PATH`.
...
plugin tools (like `lobster`), Jar4 keeps core tools enabled. To restrict core
