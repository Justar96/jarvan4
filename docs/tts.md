...
Jar4 can convert outbound replies into audio using ElevenLabs, OpenAI, or Edge TTS.
It works anywhere Jar4 can send audio; Telegram gets a round voice-note bubble.
...
Edge TTS does **not** require an API key. If no API keys are found, Jar4 defaults
...
TTS config lives under `messages.tts` in `jar4.json`.
...
- If `provider` is **unset**, Jar4 prefers `openai` (if key), then `elevenlabs` (if key),
  otherwise `edge`.
...
`~/.jar4/settings/tts.json`, override with `JAR4_TTS_PREFS` or
...
- **Edge TTS**: uses `edge.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - If the configured Edge output format fails, Jar4 retries with MP3.
...
Discord note: `/tts` is a built-in Discord command, so Jar4 registers
