...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Jar4
---

# Node + tsx "__name is not a function" crash

## Summary
Running Jar4 via Node with `tsx` fails at startup with:

```
[jar4] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `jar4 status` and `gateway:watch` worked.
