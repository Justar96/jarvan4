---
description: Update Jar4 from upstream when branch has diverged (ahead/behind)
---

# Jar4 Upstream Sync Workflow
...
# Move old version
mv /Applications/Jar4.app /tmp/Jar4-backup.app

# Install new build
cp -R dist/Jar4.app /Applications/

# Launch
open /Applications/Jar4.app
```
...
ps aux | grep -E "(jar4|gateway)" | grep -v grep

# Test agent functionality by sending a verification message
pnpm jar4 agent --message "Verification: macOS app rebuild successful - agent is responding." --session-id YOUR_TELEGRAM_SESSION_ID
...
# Update jar4.json with fallback chains
...
pnpm jar4 agent --message "Verification: Upstream sync and macOS rebuild completed successfully." --session-id YOUR_TELEGRAM_SESSION_ID || echo "Warning: Agent test failed - check Telegram for verification message"
