/**
 * Twitch channel plugin for Jar4.
 *
...
import { buildChannelConfigSchema } from "jar4/plugin-sdk";
 ...
 * for Jar4. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: Jar4Config): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: Jar4Config, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: Jar4Config): boolean => {
