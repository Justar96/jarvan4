...
export const bluebubblesMessageActions: ChannelMessageActionAdapter = {
  listActions: ({ cfg }) => {
    const account = resolveBlueBubblesAccount({ cfg: cfg as Jar4Config });
    if (!account.enabled || !account.configured) return [];
    const gate = createActionGate((cfg as Jar4Config).channels?.bluebubbles?.actions);
  ...
  handleAction: async ({ action, params, cfg, accountId, toolContext }) => {
    ...
    const opts = { cfg: cfg as Jar4Config, accountId: accountId ?? undefined };
