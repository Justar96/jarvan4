...
function resolveJar4Root(): string {
  if (coreRootCache) return coreRootCache;
  const override = process.env.JAR4_ROOT?.trim() || process.env.JAR4_ROOT?.trim();
  if (override) {
  ...
  throw new Error(
    "Unable to resolve core root. Set JAR4_ROOT (or legacy JAR4_ROOT) to the package root.",
  );
}

async function importCoreModule<T>(relativePath: string): Promise<T> {
  const root = resolveJar4Root();
