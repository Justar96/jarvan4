...
Jar4 extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
jar4 channels login --channel zalouser
...
```bash
jar4 message send --channel zalouser --target <threadId> --message "Hello from Jar4!"
```
...
```bash
jar4 channels login --channel zalouser              # Login via QR
jar4 channels login --channel zalouser --account work
jar4 channels status --probe
jar4 channels logout --channel zalouser
```
...
```bash
# Text
jar4 message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
jar4 message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `jar4 channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
jar4 channels login --channel zalouser --account work
jar4 message send --channel zalouser --account work --target <id> --message "Hello"
