...
`clawd.invoke --each`) without adding new Jar4 code per workflow.
...
This extension depends on Jar4 internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Jar4 extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.jar4/extensions` as a standalone plugin directory.
