...
// NOTE: This extension is intended to be bundled with Jar4.
// When running from source (tests/dev), Jar4 internals live under src/.
...
export function createLlmTaskTool(api: Jar4PluginApi) {
  return {
    name: "llm-task",
    description:
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "jar4-llm-task-"));
