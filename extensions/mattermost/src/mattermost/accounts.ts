...
export function listMattermostAccountIds(cfg: Jar4Config): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultMattermostAccountId(cfg: Jar4Config): string {
...
export function listEnabledMattermostAccounts(cfg: Jar4Config): ResolvedMattermostAccount[] {
