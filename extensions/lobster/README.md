...
- This plugin integrates Lobster with Jar4 *without core changes*.
...
## Using `clawd.invoke` (Lobster → Jar4 tools)

Some Lobster pipelines may include a `clawd.invoke` step to call back into Jar4 tools/plugins (for example: `gog` for Google Workspace, `gh` for GitHub, `message.send`, etc.).

For this to work, the Jar4 Gateway must expose the tool bridge endpoint and the target tool must be allowed by policy:

- Jar4 provides an HTTP endpoint: `POST /tools/invoke`.
- The request is gated by **gateway auth** (e.g. `Authorization: Bearer …` when token auth is enabled).
- The invoked tool is gated by **tool policy** (global + per-agent + provider + group policy). If the tool is not allowed, Jar4 returns `404 Tool not available`.
