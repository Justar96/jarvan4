...
export const zaloDock: ChannelDock = {
  ...
  config: {
    resolveAllowFrom: ({ cfg, accountId }) =>
      (resolveZaloAccount({ cfg: cfg as Jar4Config, accountId }).config.allowFrom ?? []).map(
        (entry) => String(entry),
      ),
...
export const zaloPlugin: ChannelPlugin<ResolvedZaloAccount> = {
  ...
  config: {
    listAccountIds: (cfg) => listZaloAccountIds(cfg as Jar4Config),
    resolveAccount: (cfg, accountId) => resolveZaloAccount({ cfg: cfg as Jar4Config, accountId }),
    defaultAccountId: (cfg) => resolveDefaultZaloAccountId(cfg as Jar4Config),
  ...
  security: {
    resolveDmPolicy: ({ cfg, accountId, account }) => {
      ...
      const useAccountPath = Boolean(
        (cfg as Jar4Config).channels?.zalo?.accounts?.[resolvedAccountId],
      );
  ...
  directory: {
    ...
    listPeers: async ({ cfg, accountId, query, limit }) => {
      const account = resolveZaloAccount({ cfg: cfg as Jar4Config, accountId });
  ...
  pairing: {
    ...
    notifyApproval: async ({ cfg, id }) => {
      const account = resolveZaloAccount({ cfg: cfg as Jar4Config });
