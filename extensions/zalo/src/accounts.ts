...
export function listZaloAccountIds(cfg: Jar4Config): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultZaloAccountId(cfg: Jar4Config): string {
...
function mergeZaloAccountConfig(cfg: Jar4Config, accountId: string): ZaloAccountConfig {
...
export function listEnabledZaloAccounts(cfg: Jar4Config): ResolvedZaloAccount[] {
