...
import type { PluginRuntime } from "jar4/plugin-sdk";
...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.JAR4_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "jar4-nostr-"));
  process.env.JAR4_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const override = env.JAR4_STATE_DIR?.trim();
        if (override) return override;
        return path.join(homedir(), ".jar4");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) delete process.env.JAR4_STATE_DIR;
    else process.env.JAR4_STATE_DIR = previous;
