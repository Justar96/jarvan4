...
import { GoogleChatConfigSchema } from "jar4/plugin-sdk";
...
export const googlechatDock: ChannelDock = {
  ...
  config: {
    resolveAllowFrom: ({ cfg, accountId }) =>
      (resolveGoogleChatAccount({ cfg: cfg as Jar4Config, accountId }).config.dm?.allowFrom ??
        []
...
export const googlechatPlugin: ChannelPlugin<ResolvedGoogleChatAccount> = {
  ...
  pairing: {
    ...
    notifyApproval: async ({ cfg, id }) => {
      const account = resolveGoogleChatAccount({ cfg: cfg as Jar4Config });
  ...
  config: {
    listAccountIds: (cfg) => listGoogleChatAccountIds(cfg as Jar4Config),
    resolveAccount: (cfg, accountId) =>
      resolveGoogleChatAccount({ cfg: cfg as Jar4Config, accountId }),
    defaultAccountId: (cfg) => resolveDefaultGoogleChatAccountId(cfg as Jar4Config),
  ...
  security: {
    resolveDmPolicy: ({ cfg, accountId, account }) => {
      ...
      const useAccountPath = Boolean(
        (cfg as Jar4Config).channels?.["googlechat"]?.accounts?.[resolvedAccountId],
      );
