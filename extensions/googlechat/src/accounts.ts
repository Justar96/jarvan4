...
export function listGoogleChatAccountIds(cfg: Jar4Config): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultGoogleChatAccountId(cfg: Jar4Config): string {
...
export function listEnabledGoogleChatAccounts(cfg: Jar4Config): ResolvedGoogleChatAccount[] {
