...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `jar4-e2e-${name}-`));
  const configDir = path.join(homeDir, ".jar4");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          HOME: homeDir,
          JAR4_CONFIG_PATH: configPath,
          JAR4_STATE_DIR: stateDir,
          JAR4_GATEWAY_TOKEN: "",
          JAR4_GATEWAY_PASSWORD: "",
          JAR4_SKIP_CHANNELS: "1",
          JAR4_SKIP_BROWSER_CONTROL_SERVER: "1",
...
const waitForNodeStatus = async (inst: GatewayInstance, nodeId: string, timeoutMs = 10_000) => {
  ...
  while (Date.now() < deadline) {
    const list = (await runCliJson(
      ...
      {
        JAR4_GATEWAY_TOKEN: inst.gatewayToken,
        JAR4_GATEWAY_PASSWORD: "",
      },
    )) as NodeListPayload;
    ...
    async () => {
      ...
      const [healthA, healthB] = (await Promise.all([
        runCliJson(["health", "--json", "--timeout", "10000"], {
          JAR4_GATEWAY_PORT: String(gwA.port),
          JAR4_GATEWAY_TOKEN: gwA.gatewayToken,
          JAR4_GATEWAY_PASSWORD: "",
        }),
        runCliJson(["health", "--json", "--timeout", "10000"], {
          JAR4_GATEWAY_PORT: String(gwB.port),
          JAR4_GATEWAY_TOKEN: gwB.gatewayToken,
          JAR4_GATEWAY_PASSWORD: "",
        }),
