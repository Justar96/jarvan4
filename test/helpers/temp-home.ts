...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("JAR4_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  process.env.JAR4_STATE_DIR = path.join(base, ".jar4");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "jar4-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".jar4", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
