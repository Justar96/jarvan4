...
import type { Jar4Config } from "../src/config/config.js";
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("jar4-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("jar4-bin-sherpa-");
      const modelDir = await makeTempDir("jar4-sherpa-model-");
      ...
      const cfg: Jar4Config = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("jar4-bin-whispercpp-");
      const modelDir = await makeTempDir("jar4-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("jar4-bin-gemini-");
      ...
      const cfg: Jar4Config = { tools: { media: { image: {} } } };
