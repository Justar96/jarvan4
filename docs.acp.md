...
This document describes how the Jar4 ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`jar4 acp` exposes an ACP agent over stdio and forwards prompts to a running
Jar4 Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a Jar4 Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `jar4 acp` over stdio.

Example config:

```bash
jar4 config set gateway.remote.url wss://gateway-host:18789
jar4 config set gateway.remote.token <token>
```

Example run:

```bash
jar4 acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
jar4 acp --session agent:main:main
jar4 acp --session agent:design:main
jar4 acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “Jar4 ACP” to start a thread.

## Execution Model

- ACP client spawns `jar4 acp` and speaks ACP messages over stdio.
...
```bash
jar4 acp --session agent:main:main
jar4 acp --session-label "support inbox"
jar4 acp --reset-session
```
...
`jar4 acp` resolves the Gateway URL and auth from CLI flags or config:
