...
function setSlackDmPolicy(cfg: Jar4Config, dmPolicy: DmPolicy) {
  const allowFrom =
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "Jar4";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for Jar4`,
    },
    features: {
      bot_user: {
...
function setSlackAllowFrom(cfg: Jar4Config, allowFrom: string[]): Jar4Config {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
