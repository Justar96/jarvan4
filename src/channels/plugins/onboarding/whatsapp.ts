...
function setWhatsAppDmPolicy(cfg: Jar4Config, dmPolicy: DmPolicy): Jar4Config {
  return mergeWhatsAppConfig(cfg, { dmPolicy });
}

function setWhatsAppAllowFrom(cfg: Jar4Config, allowFrom?: string[]): Jar4Config {
  return mergeWhatsAppConfig(cfg, { allowFrom }, { unsetOnUndefined: ["allowFrom"] });
}

function setWhatsAppSelfChatMode(cfg: Jar4Config, selfChatMode: boolean): Jar4Config {
...
async function detectWhatsAppLinked(cfg: Jar4Config, accountId: string): Promise<boolean> {
...
async function promptWhatsAppAllowFrom(
  ...
  if (options?.forceAllowlist) {
    await prompter.note(
      "We need the sender/owner number so Jar4 can allowlist you.",
      "WhatsApp number",
    );
  ...
  const phoneMode = (await prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for Jar4" },
    ],
...
export const whatsappOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({
    ...
    if (wantsLink) {
      ...
      await prompter.note(
        `Run \`${formatCliCommand("jar4 channels login")}\` later to link WhatsApp.`,
        "WhatsApp",
      );
    }
