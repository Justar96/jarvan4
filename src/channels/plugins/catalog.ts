...
import { discoverJar4Plugins } from "../../plugins/discovery.js";
import type { PluginOrigin } from "../../plugins/types.js";
import type { Jar4PackageManifest } from "../../plugins/manifest.js";
...
type ExternalCatalogEntry = {
  ...
  version?: string;
  description?: string;
  jar4?: Jar4PackageManifest;
  [LEGACY_MANIFEST_KEY]?: Jar4PackageManifest;
};
...
const ENV_CATALOG_PATHS = ["JAR4_PLUGIN_CATALOG_PATHS", "JAR4_MPM_CATALOG_PATHS"];
...
function toChannelMeta(params: {
  channel: NonNullable<Jar4PackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
...
function buildCatalogEntry(candidate: {
  ...
  packageDir?: string;
  workspaceDir?: string;
  packageJar4?: Jar4PackageManifest;
}): ChannelPluginCatalogEntry | null {
  const manifest = candidate.packageJar4;
...
function buildExternalCatalogEntry(entry: ExternalCatalogEntry): ChannelPluginCatalogEntry | null {
  const manifest = entry.jar4 ?? entry[LEGACY_MANIFEST_KEY];
  return buildCatalogEntry({
    packageName: entry.name,
    packageJar4: manifest,
  });
}
...
export function listChannelPluginCatalogEntries(
  ...
  const discovery = discoverJar4Plugins({ workspaceDir: options.workspaceDir });
