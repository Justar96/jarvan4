...
describe("media store", () => {
  ...
  const snapshotEnv = () => {
    for (const key of ["HOME", "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "JAR4_STATE_DIR"]) {
  ...
  beforeAll(async () => {
    snapshotEnv();
    home = await fs.mkdtemp(path.join(os.tmpdir(), "jar4-test-home-"));
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.JAR4_STATE_DIR = path.join(home, ".jar4");
    ...
    await fs.mkdir(path.join(home, ".jar4"), { recursive: true });
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.jar4${path.sep}media`);
