...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.JAR4_GATEWAY_PORT).toBe("18789");
    expect(env.JAR4_GATEWAY_TOKEN).toBe("secret");
    expect(env.JAR4_SERVICE_MARKER).toBe("jar4");
    expect(env.JAR4_SERVICE_KIND).toBe("gateway");
    expect(typeof env.JAR4_SERVICE_VERSION).toBe("string");
    expect(env.JAR4_SYSTEMD_UNIT).toBe("jar4-gateway.service");
    if (process.platform === "darwin") {
      expect(env.JAR4_LAUNCHD_LABEL).toBe("com.jar4.gateway");
    }
  });

  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", JAR4_PROFILE: "work" },
      port: 18789,
    });
    expect(env.JAR4_SYSTEMD_UNIT).toBe("jar4-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.JAR4_LAUNCHD_LABEL).toBe("com.jar4.work");
    }
  });
});
