...
function resolveTaskName(env: Record<string, string | undefined>): string {
  const override = env.JAR4_WINDOWS_TASK_NAME?.trim();
  if (override) return override;
  return resolveGatewayWindowsTaskName(env.JAR4_PROFILE);
}

export function resolveTaskScriptPath(env: Record<string, string | undefined>): string {
  const override = env.JAR4_TASK_SCRIPT?.trim();
  if (override) return override;
  const scriptName = env.JAR4_TASK_SCRIPT_NAME?.trim() || "gateway.cmd";
...
export async function installScheduledTask({
  ...
  const taskDescription =
    ...
    formatGatewayServiceDescription({
      profile: env.JAR4_PROFILE,
      version: environment?.JAR4_SERVICE_VERSION ?? env.JAR4_SERVICE_VERSION,
    });
