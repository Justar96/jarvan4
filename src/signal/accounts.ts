...
export function listSignalAccountIds(cfg: Jar4Config): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultSignalAccountId(cfg: Jar4Config): string {
...
function mergeSignalAccountConfig(cfg: Jar4Config, accountId: string): SignalAccountConfig {
...
export function listEnabledSignalAccounts(cfg: Jar4Config): ResolvedSignalAccount[] {
