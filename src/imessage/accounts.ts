...
export function listIMessageAccountIds(cfg: Jar4Config): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultIMessageAccountId(cfg: Jar4Config): string {
...
function mergeIMessageAccountConfig(cfg: Jar4Config, accountId: string): IMessageAccountConfig {
...
export function listEnabledIMessageAccounts(cfg: Jar4Config): ResolvedIMessageAccount[] {
