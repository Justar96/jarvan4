...
export function triggerJar4Restart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.JAR4_SYSTEMD_UNIT,
        process.env.JAR4_PROFILE,
      );
  ...
  const label =
    process.env.JAR4_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.JAR4_PROFILE);
