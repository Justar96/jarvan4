...
export const WIDE_AREA_DISCOVERY_DOMAIN = "jar4.internal.";
export const WIDE_AREA_ZONE_FILENAME = "jar4.internal.db";
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*jar4-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "jar4";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "jar4");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "jar4-gw");
  ...
  records.push(`_jar4-gw._tcp IN PTR ${instanceLabel}._jar4-gw._tcp`);
  records.push(`${instanceLabel}._jar4-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._jar4-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; jar4-content-hash: ${contentHash}\n${contentBody}`;
}
