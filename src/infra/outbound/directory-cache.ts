...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: Jar4Config | null = null;

  constructor(private readonly ttlMs: number) {}

  get(key: string, cfg: Jar4Config): T | undefined {
  ...
  set(key: string, value: T, cfg: Jar4Config): void {
    this.resetIfConfigChanged(cfg);
  ...
  clear(cfg?: Jar4Config): void {
    this.cache.clear();
    if (cfg) this.lastConfigRef = cfg;
  }

  private resetIfConfigChanged(cfg: Jar4Config): void {
