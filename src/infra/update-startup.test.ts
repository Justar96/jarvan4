...
describe("update-startup", () => {
  ...
  beforeEach(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "jar4-update-check-"));
  ...
  it("logs update hint for npm installs when newer tag exists", async () => {
    const { resolveJar4PackageRoot } = await import("./jar4-root.js");
    ...
    vi.mocked(resolveJar4PackageRoot).mockResolvedValue("/opt/jar4");
