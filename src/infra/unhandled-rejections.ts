...
export function isUnhandledRejectionHandled(reason: unknown): boolean {
  for (const handler of handlers) {
    try {
      ...
      console.error(
        "[jar4] Unhandled rejection handler failed:",
...
export function installUnhandledRejectionHandler(): void {
  process.on("unhandledRejection", (reason, _promise) => {
    ...
    if (isAbortError(reason)) {
      console.warn("[jar4] Suppressed AbortError:", formatUncaughtError(reason));
      return;
    }
    ...
    if (isTransientNetworkError(reason)) {
      console.error("[jar4] Network error (non-fatal):", formatUncaughtError(reason));
      return;
    }

    console.error("[jar4] Unhandled promise rejection:", formatUncaughtError(reason));
    process.exit(1);
  });
}
