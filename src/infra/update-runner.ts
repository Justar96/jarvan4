...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "jar4-update-preflight-"));
    ...
    const doctorStep = await runStep(
      step(
        "jar4 doctor",
        managerScriptArgs(manager, "jar4", ["doctor", "--non-interactive"]),
        gitRoot,
        { JAR4_UPDATE_IN_PROGRESS: "1" },
      ),
    );
