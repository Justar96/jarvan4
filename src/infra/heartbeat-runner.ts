...
function resolveActiveHoursTimezone(cfg: Jar4Config, raw?: string): string {
...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: Jar4Config) => void;
};

function hasExplicitHeartbeatAgents(cfg: Jar4Config) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: Jar4Config, agentId?: string): boolean {
...
function resolveHeartbeatConfig(cfg: Jar4Config, agentId?: string): HeartbeatConfig | undefined {
...
function resolveHeartbeatAgents(cfg: Jar4Config): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: Jar4Config, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: Jar4Config, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: Jar4Config) => {
    if (state.stopped) return;
    const now = Date.now();
