...
function candidateBinDirs(opts: EnsureJar4PathOpts): string[] {
  ...
  // Bundled macOS app: `jar4` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingJar4 = path.join(execDir, "jar4");
    if (isExecutable(siblingJar4)) candidates.push(execDir);
  } catch {
    // ignore
  }

  // Project-local installs (best effort): if a `node_modules/.bin/jar4` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "jar4"))) candidates.push(localBinDir);
...
/**
 * Best-effort PATH bootstrap so skills that require the `jar4` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureJar4CliOnPath(opts: EnsureJar4PathOpts = {}) {
  if (isTruthyEnvValue(process.env.JAR4_PATH_BOOTSTRAPPED)) return;
  process.env.JAR4_PATH_BOOTSTRAPPED = "1";
