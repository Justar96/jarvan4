  ...
  try {
    if (boxes.length > 0) {
      await page.evaluate((labels) => {
        const existing = document.querySelectorAll("[data-jar4-labels]");
        existing.forEach((el) => el.remove());

        const root = document.createElement("div");
        root.setAttribute("data-jar4-labels", "1");
        ...
        for (const label of labels) {
          ...
          box.setAttribute("data-jar4-labels", "1");
          ...
          tag.setAttribute("data-jar4-labels", "1");
