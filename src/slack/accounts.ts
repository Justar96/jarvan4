...
export function listSlackAccountIds(cfg: Jar4Config): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultSlackAccountId(cfg: Jar4Config): string {
...
function mergeSlackAccountConfig(cfg: Jar4Config, accountId: string): SlackAccountConfig {
...
export function listEnabledSlackAccounts(cfg: Jar4Config): ResolvedSlackAccount[] {
