...
`;
    ...
    const parsed = JSON5.parse(result.metadata ?? "") as { jar4?: { emoji?: string } };
    expect(parsed.jar4?.emoji).toBe("disk");
  });
...
`;
    ...
    const parsed = JSON5.parse(result.metadata ?? "") as { jar4?: { events?: string[] } };
