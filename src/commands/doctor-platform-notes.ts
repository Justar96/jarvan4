...
export async function noteMacLaunchAgentOverrides() {
  ...
  const markerPath = path.join(resolveHomeDir(), ".jar4", "disable-launchagent");
...
function hasConfigGatewayCreds(cfg: Jar4Config): boolean {
  const localToken =
...
export async function noteMacLaunchctlGatewayEnvOverrides(
  ...
  const envToken = await getenv("JAR4_GATEWAY_TOKEN");
  const envPassword = await getenv("JAR4_GATEWAY_PASSWORD");
  if (!envToken && !envPassword) return;

  const lines = [
    "- launchctl environment overrides detected (can cause confusing unauthorized errors).",
    envToken ? "- `JAR4_GATEWAY_TOKEN` is set; it overrides config tokens." : undefined,
    envPassword
      ? "- `JAR4_GATEWAY_PASSWORD` is set; it overrides config passwords."
      : undefined,
    "- Clear overrides and restart the app/gateway:",
    envToken ? "  launchctl unsetenv JAR4_GATEWAY_TOKEN" : undefined,
    envPassword ? "  launchctl unsetenv JAR4_GATEWAY_PASSWORD" : undefined,
  ].filter((line): line is string => Boolean(line));
