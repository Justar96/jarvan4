...
export async function modelsStatusCommand(
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("jar4 models auth setup-token")}\` or \`${formatCliCommand("jar4 configure")}\`.`
          : `Run \`${formatCliCommand("jar4 configure")}\` or set an API key env var.`;
