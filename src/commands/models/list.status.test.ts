...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveJar4AgentDir: vi.fn().mockReturnValue("/tmp/jar4-agent"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveJar4AgentDir: mocks.resolveJar4AgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(payload.auth.storePath).toBe("/tmp/jar4-agent/auth-profiles.json");
