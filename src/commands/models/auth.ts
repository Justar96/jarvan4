...
import { readConfigFileSnapshot, type Jar4Config } from "../../config/config.js";
...
function applyDefaultModel(cfg: Jar4Config, model: string): Jar4Config {
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (providers.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("jar4 plugins install")}\`.`,
    );
  }
