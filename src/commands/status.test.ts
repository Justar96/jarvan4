...
beforeAll(() => {
  previousProfile = process.env.JAR4_PROFILE;
  process.env.JAR4_PROFILE = "isolated";
});
...
afterAll(() => {
  if (previousProfile === undefined) {
    ...
    process.env.JAR4_PROFILE = previousProfile;
  }
});
...
vi.mock("../infra/jar4-root.js", () => ({
  resolveJar4PackageRoot: vi.fn().mockResolvedValue("/tmp/jar4"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/jar4/pnpm-lock.yaml",
      markerPath: "/tmp/jar4/node_modules/.modules.yaml",
    },
...
vi.mock("../daemon/service.js", () => ({
  resolveGatewayService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/com.jar4.gateway.plist",
    }),
  }),
}));
vi.mock("../daemon/node-service.js", () => ({
  resolveNodeService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/com.jar4.node.plist",
    }),
  }),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(logs.some((l) => l.includes("Jar4 status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("jar4 status --all") ||
          l.includes("jar4 --profile isolated status --all") ||
          l.includes("jar4 status --all") ||
          l.includes("jar4 --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.JAR4_GATEWAY_TOKEN = "abcd1234";
    try {
