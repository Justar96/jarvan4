...
const requireAgentDir = () => {
  const agentDir = process.env.JAR4_AGENT_DIR;
  if (!agentDir) throw new Error("JAR4_AGENT_DIR not set");
  return agentDir;
};
...
describe("writeOAuthCredentials", () => {
  ...
  afterEach(async () => {
    ...
    delete process.env.JAR4_OAUTH_DIR;
  });

  it("writes auth-profiles.json under JAR4_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "jar4-oauth-"));
    process.env.JAR4_STATE_DIR = tempStateDir;
    process.env.JAR4_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.JAR4_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to JAR4_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "jar4-minimax-"));
    process.env.JAR4_STATE_DIR = tempStateDir;
    process.env.JAR4_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.JAR4_AGENT_DIR;
