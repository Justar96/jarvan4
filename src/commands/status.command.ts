...
export async function statusCommand(
  ...
  runtime.log(theme.heading("Jar4 status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("jar4 security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("jar4 security audit --deep")}`));
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("jar4 status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("jar4 logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("jar4 status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("jar4 gateway probe")}`);
  }
}
