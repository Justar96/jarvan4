  ...
  it("warns when extra workspace directories exist", async () => {
    ...
    expect(
      note.mock.calls.some(
        ([message, title]) =>
          title === "Extra workspace" && typeof message === "string" && message.includes("jar4"),
      ),
    ).toBe(true);
