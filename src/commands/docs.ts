...
const SEARCH_TOOL = "https://docs.molt.bot/mcp.SearchJar4";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    ...
    if (isRich()) {
      ...
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('jar4 docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.molt.bot/");
      runtime.log(`Search: ${formatCliCommand('jar4 docs "your query"')}`);
    }
    return;
  }
