...
const debugHealth = (...args: unknown[]) => {
  if (isTruthyEnvValue(process.env.JAR4_DEBUG_HEALTH)) {
...
export async function healthCommand(
  opts: { json?: boolean; timeoutMs?: number; verbose?: boolean; config?: Jar4Config },
  runtime: RuntimeEnv,
) {
  ...
  if (opts.json) {
    ...
    const debugEnabled = isTruthyEnvValue(process.env.JAR4_DEBUG_HEALTH);
    if (opts.verbose) {
