...
function resolveDefaultAccountId(cfg: Jar4Config, provider: ChannelId): string {
  const plugin = getChannelPlugin(provider);
  if (!plugin) return DEFAULT_ACCOUNT_ID;
  return resolveChannelDefaultAccountId({ plugin, cfg });
}

function shouldShowProviderEntry(entry: ProviderAccountStatus, cfg: Jar4Config): boolean {
...
export function summarizeBindings(cfg: Jar4Config, bindings: AgentBinding[]): string[] {
