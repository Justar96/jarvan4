...
export function listAgentEntries(cfg: Jar4Config): AgentEntry[] {
...
function resolveAgentName(cfg: Jar4Config, agentId: string) {
...
function resolveAgentModel(cfg: Jar4Config, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: Jar4Config): AgentSummary[] {
