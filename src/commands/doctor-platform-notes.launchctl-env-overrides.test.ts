...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "JAR4_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("JAR4_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv JAR4_GATEWAY_TOKEN");
    expect(message).not.toContain("JAR4_GATEWAY_PASSWORD");
  });
