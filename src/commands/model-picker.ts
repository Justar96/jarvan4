...
function resolveConfiguredModelRaw(cfg: Jar4Config): string {
  const raw = cfg.agents?.defaults?.model as { primary?: string } | string | undefined;
  if (typeof raw === "string") return raw.trim();
  return raw?.primary?.trim() ?? "";
}

function resolveConfiguredModelKeys(cfg: Jar4Config): string[] {
...
export function applyPrimaryModel(cfg: Jar4Config, model: string): Jar4Config {
...
export function applyModelAllowlist(cfg: Jar4Config, models: string[]): Jar4Config {
  const defaults = cfg.agents?.defaults;
