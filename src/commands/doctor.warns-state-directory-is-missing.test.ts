...
beforeEach(() => {
  ...
  resolveJar4PackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.JAR4_STATE_DIR;
  originalUpdateInProgress = process.env.JAR4_UPDATE_IN_PROGRESS;
  process.env.JAR4_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "jar4-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.JAR4_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.JAR4_UPDATE_IN_PROGRESS;
  } else {
    process.env.JAR4_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
...
const resolveJar4PackageRoot = vi.fn().mockResolvedValue(null);
...
vi.mock("../plugins/loader.js", () => ({
  loadJar4Plugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../infra/jar4-root.js", () => ({
  resolveJar4PackageRoot,
}));
...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "jar4-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.JAR4_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when JAR4_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/jar4.json",
      exists: true,
      raw: "{}",
    ...
    process.env.JAR4_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
