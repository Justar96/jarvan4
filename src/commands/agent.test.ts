...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "jar4-agent-" });
}

function mockConfig(
  home: string,
  storePath: string,
  agentOverrides?: Partial<NonNullable<NonNullable<Jar4Config["agents"]>["defaults"]>>,
  telegramOverrides?: Partial<NonNullable<Jar4Config["telegram"]>>,
