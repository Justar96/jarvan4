...
import type { Jar4Config, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: Jar4Config) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("jar4 security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("jar4 configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("jar4 config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("jar4 doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("jar4 config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
