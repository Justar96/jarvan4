...
describe("onboard (non-interactive): gateway and remote auth", () => {
  const prev = {
    ...
    password: process.env.JAR4_GATEWAY_PASSWORD,
  };
  ...
  const initStateDir = async (prefix: string) => {
    ...
    process.env.JAR4_STATE_DIR = stateDir;
  ...
  beforeAll(async () => {
    ...
    tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "jar4-onboard-"));
  ...
  afterAll(async () => {
    ...
    process.env.JAR4_GATEWAY_PASSWORD = prev.password;
  });
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    process.env.JAR4_STATE_DIR = stateDir;
    process.env.JAR4_CONFIG_PATH = path.join(stateDir, "jar4.json");
