...
async function detectJar4GitCheckout(root: string): Promise<"git" | "not-git" | "unknown"> {
...
export async function maybeOfferUpdateBeforeDoctor(params: {
  ...
  const updateInProgress = isTruthyEnvValue(process.env.JAR4_UPDATE_IN_PROGRESS);
  ...
  const git = await detectJar4GitCheckout(params.root);
  if (git === "git") {
    const shouldUpdate = await params.confirm({
      message: "Update Jar4 from git before running doctor?",
      initialValue: true,
    });
  ...
  if (git === "not-git") {
    note(
      [
        ...
        `Run \`${formatCliCommand("jar4 update")}\` to update via your package manager (npm/pnpm), then rerun doctor.`,
      ].join("\n"),
      "Update",
    );
  }
