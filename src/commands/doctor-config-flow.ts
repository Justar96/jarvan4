...
function stripUnknownConfigKeys(config: Jar4Config): {
  config: Jar4Config;
  removed: string[];
} {
  const parsed = Jar4Schema.safeParse(config);
  if (parsed.success) {
    return { config, removed: [] };
  }

  const next = structuredClone(config) as Jar4Config;
...
function noteOpencodeProviderOverrides(cfg: Jar4Config) {
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  let cfg: Jar4Config = baseCfg;
  let candidate = structuredClone(baseCfg) as Jar4Config;
  ...
  if (snapshot.legacyIssues.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(
        `Run "${formatCliCommand("jar4 doctor --fix")}" to apply legacy migrations.`,
      );
    }
  }
  ...
  if (normalized.changes.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(`Run "${formatCliCommand("jar4 doctor --fix")}" to apply these changes.`);
    }
  }
  ...
  if (unknown.removed.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push('Run "jar4 doctor --fix" to remove these keys.');
    }
  }
