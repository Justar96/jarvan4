...
function resolveProfileStateDir(profile: string, homedir: () => string): string {
  ...
  return path.join(homedir(), `.jar4${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.JAR4_PROFILE = profile;

  const stateDir = env.JAR4_STATE_DIR?.trim() || resolveProfileStateDir(profile, homedir);
  if (!env.JAR4_STATE_DIR?.trim()) env.JAR4_STATE_DIR = stateDir;

  if (!env.JAR4_CONFIG_PATH?.trim()) {
    env.JAR4_CONFIG_PATH = path.join(stateDir, "jar4.json");
  }

  if (profile === "dev" && !env.JAR4_GATEWAY_PORT?.trim()) {
    env.JAR4_GATEWAY_PORT = "19001";
  }
}
