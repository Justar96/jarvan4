...
describe("daemon-cli coverage", () => {
  const originalEnv = {
    JAR4_STATE_DIR: process.env.JAR4_STATE_DIR,
    JAR4_CONFIG_PATH: process.env.JAR4_CONFIG_PATH,
    JAR4_GATEWAY_PORT: process.env.JAR4_GATEWAY_PORT,
    JAR4_PROFILE: process.env.JAR4_PROFILE,
  };

  beforeEach(() => {
    process.env.JAR4_STATE_DIR = "/tmp/jar4-cli-state";
    process.env.JAR4_CONFIG_PATH = "/tmp/jar4-cli-state/jar4.json";
    delete process.env.JAR4_GATEWAY_PORT;
    delete process.env.JAR4_PROFILE;
    serviceReadCommand.mockResolvedValue(null);
  });

  afterEach(() => {
    if (originalEnv.JAR4_STATE_DIR !== undefined)
      process.env.JAR4_STATE_DIR = originalEnv.JAR4_STATE_DIR;
    else delete process.env.JAR4_STATE_DIR;

    if (originalEnv.JAR4_CONFIG_PATH !== undefined)
      process.env.JAR4_CONFIG_PATH = originalEnv.JAR4_CONFIG_PATH;
    else delete process.env.JAR4_CONFIG_PATH;

    if (originalEnv.JAR4_GATEWAY_PORT !== undefined)
      process.env.JAR4_GATEWAY_PORT = originalEnv.JAR4_GATEWAY_PORT;
    else delete process.env.JAR4_GATEWAY_PORT;

    if (originalEnv.JAR4_PROFILE !== undefined)
      process.env.JAR4_PROFILE = originalEnv.JAR4_PROFILE;
    else delete process.env.JAR4_PROFILE;
  });
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        JAR4_PROFILE: "dev",
        JAR4_STATE_DIR: "/tmp/jar4-daemon-state",
        JAR4_CONFIG_PATH: "/tmp/jar4-daemon-state/jar4.json",
        JAR4_GATEWAY_PORT: "19001",
      },
      sourcePath: "/tmp/com.jar4.gateway.plist",
    });
