...
import { resolveJar4PackageRoot } from "../infra/jar4-root.js";
import {
  checkUpdateStatus,
...
const STEP_LABELS: Record<string, string> = {
  ...
  "jar4 doctor": "Running doctor checks",
...
const DEFAULT_PACKAGE_NAME = "jar4";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME, "jar4"]);
const CLI_NAME = resolveCliName();
const JAR4_REPO_URL = "https://github.com/jar4/jar4.git";
const DEFAULT_GIT_DIR = path.join(os.homedir(), "jar4");

function normalizeTag(value?: string | null): string | null {
  if (!value) return null;
  const trimmed = value.trim();
  if (!trimmed) return null;
  if (trimmed.startsWith("jar4@")) return trimmed.slice("jar4@".length);
...
function resolveGitInstallDir(): string {
  const override = process.env.JAR4_GIT_DIR?.trim();
...
async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", JAR4_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `JAR4_GIT_DIR points at a non-git directory: ${params.dir}. Set JAR4_GIT_DIR to an empty folder or a jar4 checkout.`,
      );
    }
    return await runUpdateStep({
      name: "git clone",
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`JAR4_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function updateStatusCommand(opts: UpdateStatusOptions): Promise<void> {
  ...
  defaultRuntime.log(theme.heading("Jar4 update status"));
  defaultRuntime.log("");
  defaultRuntime.log(
    renderTable({
      width: tableWidth,
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating Jar4..."));
    defaultRuntime.log("");
  }
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this Jar4 install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("jar4 doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("jar4 gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g jar4@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g jar4@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
  ...
  if (shouldRestart) {
    ...
    try {
      ...
      if (!opts.json && restarted) {
        ...
        defaultRuntime.log("");
        process.env.JAR4_UPDATE_IN_PROGRESS = "1";
        try {
          ...
          delete process.env.JAR4_UPDATE_IN_PROGRESS;
        }
      }
    } catch (err) {
      if (!opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("jar4 gateway restart"), CLI_NAME)}`,
          ),
        );
      }
    }
    ...
    if (result.mode === "npm" || result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("jar4 doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("jar4 gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("jar4 gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
export async function updateWizardCommand(opts: UpdateWizardOptions = {}): Promise<void> {
  if (!process.stdin.isTTY) {
    defaultRuntime.error(
      "Update wizard requires a TTY. Use `jar4 update --channel <stable|beta|dev>` instead.",
    );
    defaultRuntime.exit(1);
    return;
  }
  ...
  if (requestedChannel === "dev" && updateStatus.installKind !== "git") {
    ...
    if (!hasGit) {
      ...
      if (dirExists) {
        ...
        if (!empty) {
          defaultRuntime.error(
            `JAR4_GIT_DIR points at a non-git directory: ${gitDir}. Set JAR4_GIT_DIR to an empty folder or a jar4 checkout.`,
          );
          defaultRuntime.exit(1);
          return;
        }
      ...
      const ok = await confirm({
        message: stylePromptMessage(
          `Create a git checkout at ${gitDir}? (override via JAR4_GIT_DIR)`,
        ),
        initialValue: true,
      });
      if (isCancel(ok) || ok === false) {
...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update Jar4 to the latest version")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["jar4 update", "Update a source checkout (git)"],
        ["jar4 update --channel beta", "Switch to beta channel (git + npm)"],
        ["jar4 update --channel dev", "Switch to dev channel (git + npm)"],
        ["jar4 update --tag beta", "One-off update to a dist-tag or version"],
        ["jar4 update --no-restart", "Update without restarting the service"],
        ["jar4 update --json", "Output result as JSON"],
        ["jar4 update --yes", "Non-interactive (accept downgrade prompts)"],
        ["jar4 update wizard", "Interactive update wizard"],
        ["jar4 --update", "Shorthand for jar4 update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run jar4 update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.molt.bot/cli/update")}`;
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["jar4 update status", "Show channel + version status."],
          ["jar4 update status --json", "JSON output."],
          ["jar4 update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
