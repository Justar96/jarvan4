...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("jar4 node install"), formatCliCommand("jar4 node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("jar4 node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    JAR4_LOG_PREFIX: baseEnv.JAR4_LOG_PREFIX ?? "node",
