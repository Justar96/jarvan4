...
describe("gateway SIGTERM", () => {
  ...
  it("exits 0 on SIGTERM", { timeout: 180_000 }, async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "jar4-gateway-test-"));
    const out: string[] = [];
    const err: string[] = [];
    ...
    const env = {
      ...
      JAR4_SKIP_GMAIL_WATCHER: "1",
      JAR4_SKIP_CRON: "1",
      JAR4_SKIP_BROWSER_CONTROL_SERVER: "1",
      JAR4_SKIP_CANVAS_HOST: "1",
    };
    const bootstrapPath = path.join(stateDir, "jar4-entry-bootstrap.mjs");
