...
export async function installChromeExtension(opts?: {
  ...
  if (!hasManifest(src)) {
    throw new Error("Bundled Chrome extension is missing. Reinstall Jar4 and try again.");
  }
      ...
      defaultRuntime.error(
        info(
          [
            ...
            `- Pin “Jar4 Browser Relay”, then click it on the tab (badge shows ON)`,
            "",
    ...
    .action(async (_opts, cmd) => {
      ...
      if (!hasManifest(dir)) {
        defaultRuntime.error(
          danger(
            [
              `Chrome extension is not installed. Run: "${formatCliCommand("jar4 browser extension install")}"`,
