...
declare const __JAR4_VERSION__: string | undefined;

const BUNDLED_VERSION =
  (typeof __JAR4_VERSION__ === "string" && __JAR4_VERSION__) ||
  process.env.JAR4_BUNDLED_VERSION ||
  "0.0.0";
...
async function main() {
  ...
  const { ensureJar4CliOnPath } = await import("../infra/path-env.js");
  ensureJar4CliOnPath();
  ...
  process.on("uncaughtException", (error) => {
    console.error("[jar4] Uncaught exception:", formatUncaughtError(error));
    process.exit(1);
  });
...
void main().catch((err) => {
  console.error("[jar4] Relay failed:", err instanceof Error ? (err.stack ?? err.message) : err);
  process.exit(1);
});
