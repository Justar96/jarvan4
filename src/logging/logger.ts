...
export const DEFAULT_LOG_DIR = "/tmp/jar4";
export const DEFAULT_LOG_FILE = path.join(DEFAULT_LOG_DIR, "jar4.log"); // legacy single-file path

const LOG_PREFIX = "jar4";
...
function resolveSettings(): ResolvedSettings {
  let cfg: Jar4Config["logging"] | undefined =
    (loggingState.overrideSettings as LoggerSettings | null) ?? readLoggingConfig();
  if (!cfg) {
    try {
      const loaded = requireConfig("../config/config.js") as {
        loadConfig?: () => Jar4Config;
      };
