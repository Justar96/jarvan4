...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "jar4-cron-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".jar4", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<Jar4Config> = {},
): Jar4Config {
  const base: Jar4Config = {
    agents: {
      defaults: {
...
describe("runCronIsolatedAgentTurn", () => {
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".jar4", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {
