...
const ETC_JAR4_DIR = path.join(ROOT_DIR, "etc", "jar4");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "jar4.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcJar4Path(...parts: string[]) {
  return path.join(ETC_JAR4_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcJar4Path("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "jar4.json"))).toEqual({
      shared: true,
    });
  });
});
