...
import type { Jar4Config } from "./types.js";

/**
 * Nix mode detection: When JAR4_NIX_MODE=1, the gateway is running under Nix.
 * In this mode:
...
export function resolveIsNixMode(env: NodeJS.ProcessEnv = process.env): boolean {
  return env.JAR4_NIX_MODE === "1";
}

export const isNixMode = resolveIsNixMode();

const LEGACY_STATE_DIRNAME = ".jar4";
const NEW_STATE_DIRNAME = ".jar4";
const CONFIG_FILENAME = "jar4.json";
 ...
 * Can be overridden via JAR4_STATE_DIR (preferred) or JAR4_STATE_DIR (legacy).
 * Default: ~/.jar4 (legacy default for compatibility)
 */
 ...
 * Can be overridden via JAR4_CONFIG_PATH (preferred) or JAR4_CONFIG_PATH (legacy).
 * Default: ~/.jar4/jar4.json (or $*_STATE_DIR/jar4.json)
 */
...
export function resolveDefaultConfigCandidates(
  ...
  const jar4StateDir = env.JAR4_STATE_DIR?.trim();
  if (jar4StateDir) {
 ...
 * Default: os.tmpdir()/jar4-<uid> (uid suffix when available).
 */
export function resolveGatewayLockDir(tmpdir: () => string = os.tmpdir): string {
  const base = tmpdir();
  const uid = typeof process.getuid === "function" ? process.getuid() : undefined;
  const suffix = uid != null ? `jar4-${uid}` : "jar4";
 ...
 *
 * Precedence:
 * - `JAR4_OAUTH_DIR` (explicit override)
 * - `$*_STATE_DIR/credentials` (canonical server/default)
 * - `~/.jar4/credentials` (legacy default)
 */
export function resolveOAuthDir(
  env: NodeJS.ProcessEnv = process.env,
  stateDir: string = resolveStateDir(env, os.homedir),
): string {
  const override = env.JAR4_OAUTH_DIR?.trim();
  if (override) return resolveUserPath(override);
