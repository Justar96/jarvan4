...
import type { Jar4Config, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import { validateConfigObjectWithPlugins } from "./validation.js";
import { compareJar4Versions } from "./version.js";
...
function coerceConfig(value: unknown): Jar4Config {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as Jar4Config;
}
...
function stampConfigVersion(cfg: Jar4Config): Jar4Config {
...
function warnIfConfigFromFuture(cfg: Jar4Config, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) return;
  const cmp = compareJar4Versions(VERSION, touched);
  if (cmp === null) return;
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer Jar4 (${touched}); current version is ${VERSION}.`,
    );
  }
}

function applyConfigEnv(cfg: Jar4Config, env: NodeJS.ProcessEnv): void {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): Jar4Config {
    try {
      ...
      if (resolved && typeof resolved === "object" && "env" in resolved) {
        applyConfigEnv(resolved as Jar4Config, deps.env);
      }
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as Jar4Config, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: Jar4Config) {
    clearConfigCache();
...
// module scope. `JAR4_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.JAR4_CONFIG_CACHE_MS?.trim();
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.JAR4_DISABLE_CONFIG_CACHE?.trim()) return false;
...
export function loadConfig(): Jar4Config {
...
export async function writeConfigFile(cfg: Jar4Config): Promise<void> {
  clearConfigCache();
