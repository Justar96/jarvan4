import type { Jar4Config } from "./config.js";
import {
  getChatChannelMeta,
  listChatChannels,
...
function isTelegramConfigured(cfg: Jar4Config, env: NodeJS.ProcessEnv): boolean {
...
function isDiscordConfigured(cfg: Jar4Config, env: NodeJS.ProcessEnv): boolean {
...
function isSlackConfigured(cfg: Jar4Config, env: NodeJS.ProcessEnv): boolean {
  if (
...
function isSignalConfigured(cfg: Jar4Config): boolean {
...
function isIMessageConfigured(cfg: Jar4Config): boolean {
...
function isWhatsAppConfigured(cfg: Jar4Config): boolean {
...
function isGenericChannelConfigured(cfg: Jar4Config, channelId: string): boolean {
...
function collectModelRefs(cfg: Jar4Config): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: Jar4Config, providerId: string): boolean {
...
function isPluginExplicitlyDisabled(cfg: Jar4Config, pluginId: string): boolean {
  const entry = cfg.plugins?.entries?.[pluginId];
  return entry?.enabled === false;
}

function isPluginDenied(cfg: Jar4Config, pluginId: string): boolean {
...
function enablePluginEntry(cfg: Jar4Config, pluginId: string): Jar4Config {
  const entries = {
    ...cfg.plugins?.entries,
    [pluginId]: {
