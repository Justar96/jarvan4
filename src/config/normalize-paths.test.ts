  ...
  it("expands tilde for path-ish keys only", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = normalizeConfigPaths({
        ...
        logging: { file: "~/.jar4/logs/jar4.log" },
        hooks: {
          path: "~/.jar4/hooks.json5",
      ...
      expect(cfg.logging?.file).toBe(path.join(home, ".jar4", "logs", "jar4.log"));
      expect(cfg.hooks?.path).toBe(path.join(home, ".jar4", "hooks.json5"));
      ...
      expect(cfg.channels?.telegram?.accounts?.personal?.tokenFile).toBe(
        path.join(home, ".jar4", "telegram.token"),
      );
