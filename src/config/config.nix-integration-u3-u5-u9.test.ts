...
describe("Nix integration (U3, U5, U9)", () => {
  describe("U3: isNixMode env var detection", () => {
    it("isNixMode is false when JAR4_NIX_MODE is not set", async () => {
      await withEnvOverride({ JAR4_NIX_MODE: undefined }, async () => {
        const { isNixMode } = await import("./config.js");
        expect(isNixMode).toBe(false);
      });
    });

    it("isNixMode is false when JAR4_NIX_MODE is empty", async () => {
      await withEnvOverride({ JAR4_NIX_MODE: "" }, async () => {
        const { isNixMode } = await import("./config.js");
        expect(isNixMode).toBe(false);
      });
    });

    it("isNixMode is false when JAR4_NIX_MODE is not '1'", async () => {
      await withEnvOverride({ JAR4_NIX_MODE: "true" }, async () => {
        const { isNixMode } = await import("./config.js");
        expect(isNixMode).toBe(false);
      });
    });

    it("isNixMode is true when JAR4_NIX_MODE=1", async () => {
      await withEnvOverride({ JAR4_NIX_MODE: "1" }, async () => {
        const { isNixMode } = await import("./config.js");
  ...
  describe("U5: CONFIG_PATH and STATE_DIR env var overrides", () => {
    ...
    it("CONFIG_PATH defaults to ~/.jar4/jar4.json when env not set", async () => {
      await withEnvOverride(
        ...
        async () => {
          ...
          expect(CONFIG_PATH).toMatch(/\.jar4[\\/]jar4\.json$/);
        },
      );
    });
    ...
    it("CONFIG_PATH prefers JAR4_CONFIG_PATH over legacy override", async () => {
      await withEnvOverride(
        ...
        async () => {
          const { CONFIG_PATH } = await import("./config.js");
          expect(CONFIG_PATH).toBe(path.resolve("/nix/store/new/jar4.json"));
        },
      );
    });
    ...
    it("CONFIG_PATH uses STATE_DIR when only state dir is overridden", async () => {
      await withEnvOverride(
        ...
        async () => {
          const { CONFIG_PATH } = await import("./config.js");
          expect(CONFIG_PATH).toBe(path.join(path.resolve("/custom/state"), "jar4.json"));
        },
      );
    });
  });
  ...
  describe("U5b: tilde expansion for config paths", () => {
    it("expands ~ in common path-ish config fields", async () => {
      await withTempHome(async (home) => {
        const configDir = path.join(home, ".jar4");
        ...
        await fs.writeFile(
          path.join(pluginDir, "jar4.plugin.json"),
          JSON.stringify(
            {
              id: "demo-plugin",
          ...
          JSON.stringify(
            {
              ...
              channels: {
                whatsapp: {
                  accounts: {
                    personal: {
                      authDir: "~/.jar4/credentials/wa-personal",
                    },
                  },
                },
              },
  ...
  describe("U6: gateway port resolution", () => {
    it("uses default when env and config are unset", async () => {
      await withEnvOverride({ JAR4_GATEWAY_PORT: undefined }, async () => {
        const { DEFAULT_GATEWAY_PORT, resolveGatewayPort } = await import("./config.js");
        expect(resolveGatewayPort({})).toBe(DEFAULT_GATEWAY_PORT);
      });
    });

    it("prefers JAR4_GATEWAY_PORT over config", async () => {
      await withEnvOverride({ JAR4_GATEWAY_PORT: "19001" }, async () => {
    ...
    it("falls back to config when env is invalid", async () => {
      await withEnvOverride({ JAR4_GATEWAY_PORT: "nope" }, async () => {
        const { resolveGatewayPort } = await import("./config.js");
