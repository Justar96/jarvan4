...
import type { Jar4Config, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: Jar4Config): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: Jar4Config } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = Jar4Schema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as Jar4Config);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as Jar4Config);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as Jar4Config)),
    ),
  };
}
