...
import { scheduleGatewaySigusr1Restart, triggerJar4Restart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting jar4 in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerJar4Restart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting jar4 via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
