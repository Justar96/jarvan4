...
beforeAll(async () => {
  testWorkspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "jar4-commands-"));
...
function buildParams(commandBody: string, cfg: Jar4Config, ctxOverrides?: Partial<MsgContext>) {
  const ctx = {
    Body: commandBody,
