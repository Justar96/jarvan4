...
type AgentConfig = Partial<NonNullable<NonNullable<Jar4Config["agents"]>["defaults"]>>;
...
const readUsageFromSessionLog = (
  ...
  | undefined => {
  // Transcripts are stored at the session file path (fallback: ~/.jar4/sessions/<SessionId>.jsonl)
...
export function buildStatusMessage(args: StatusArgs): string {
  ...
  const versionLine = `🦞 Jar4 ${VERSION}${commit ? ` (${commit})` : ""}`;
...
export function buildHelpMessage(cfg?: Jar4Config): string {
  const lines = ["ℹ️ Help", ""];
