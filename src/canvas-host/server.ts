...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      (window.webkit.messageHandlers.jar4CanvasA2UIAction ||
        window.webkit.messageHandlers.jar4CanvasA2UIAction)
    );
  const hasAndroid = () =>
    !!(
      (window.jar4CanvasA2UIAction &&
        typeof window.jar4CanvasA2UIAction.postMessage === "function") ||
      (window.jar4CanvasA2UIAction &&
        typeof window.jar4CanvasA2UIAction.postMessage === "function")
    );
  const legacySend = typeof window.jar4SendUserAction === "function" ? window.jar4SendUserAction : undefined;
  if (!window.jar4SendUserAction && legacySend) {
    window.jar4SendUserAction = legacySend;
  }
  if (!window.jar4SendUserAction && typeof window.jar4SendUserAction === "function") {
    window.jar4SendUserAction = window.jar4SendUserAction;
  }
  const hasHelper = () =>
    typeof window.jar4SendUserAction === "function" ||
    typeof window.jar4SendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("jar4:a2ui-action-status", (ev) => {
  ...
  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android Jar4 node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.jar4SendUserAction === "function"
        ? window.jar4SendUserAction
        : window.jar4SendUserAction;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.JAR4_SKIP_CANVAS_HOST)) return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>Jar4 Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
