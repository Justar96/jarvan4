...
import { discoverJar4Plugins, type PluginCandidate } from "./discovery.js";
...
function resolveManifestCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.JAR4_PLUGIN_MANIFEST_CACHE_MS?.trim();
...
function shouldUseManifestCache(env: NodeJS.ProcessEnv): boolean {
  const disabled = env.JAR4_DISABLE_PLUGIN_MANIFEST_CACHE?.trim();
