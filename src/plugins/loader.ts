...
import { discoverJar4Plugins } from "./discovery.js";
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: Jar4PluginDefinition;
  register?: Jar4PluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as Jar4PluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as Jar4PluginDefinition;
...
export function loadJar4Plugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverJar4Plugins({
    workspaceDir: options.workspaceDir,
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: Jar4PluginModule | null = null;
    try {
      mod = jiti(candidate.source) as Jar4PluginModule;
    } catch (err) {
