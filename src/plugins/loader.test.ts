...
import { loadJar4Plugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const tempDirs: string[] = [];
const prevBundledDir = process.env.JAR4_BUNDLED_PLUGINS_DIR;
const EMPTY_PLUGIN_SCHEMA = { type: "object", additionalProperties: false, properties: {} };

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `jar4-plugin-${randomUUID()}`);
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "jar4.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.JAR4_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});

describe("loadJar4Plugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.JAR4_BUNDLED_PLUGINS_DIR = bundledDir;

    const registry = loadJar4Plugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadJar4Plugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
