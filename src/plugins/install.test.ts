...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `jar4-plugin-install-${randomUUID()}`);
...
describe("installPluginFromArchive", () => {
  it("installs into ~/.jar4/extensions and uses unscoped id", async () => {
  ...
  it("rejects packages without jar4.extensions", async () => {
    const stateDir = makeTempDir();
    const workDir = makeTempDir();
    const pkgDir = path.join(workDir, "package");
    ...
    fs.writeFileSync(
      path.join(pkgDir, "package.json"),
      JSON.stringify({ name: "@jar4/nope", version: "0.0.1" }),
      "utf-8",
    );
    ...
    if (result.ok) return;
    expect(result.error).toContain("jar4.extensions");
  });
});
