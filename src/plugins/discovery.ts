...
function deriveIdHint(params: {
  ...
  // package is scoped (example: @jar4/voice-call -> voice-call).
...
function addCandidate(params: {
  ...
  params.candidates.push({
    ...
    packageJar4: getPackageManifestMetadata(manifest ?? undefined),
  });
}
...
export function discoverJar4Plugins(params: {
  workspaceDir?: string;
  extraPaths?: string[];
  ...
  if (workspaceDir) {
    ...
    const workspaceExt = path.join(workspaceRoot, ".jar4", "extensions");
    discoverInDirectory({
      dir: workspaceExt,
      origin: "workspace",
