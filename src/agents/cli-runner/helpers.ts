...
/**
 * Cleanup suspended Jar4 CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: Jar4Config) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "jar4-cli-images-"));
  const paths: string[] = [];
