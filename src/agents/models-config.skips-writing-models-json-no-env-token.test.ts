...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "jar4-models-" });
}

const MODELS_CONFIG: Jar4Config = {
  models: {
    providers: {
      "custom-proxy": {
...
describe("models-config", () => {
  ...
  it("skips writing models.json when no env token or profile exists", async () => {
    await withTempHome(async (home) => {
      ...
      try {
        vi.resetModules();
        const { ensureJar4ModelsJson } = await import("./models-config.js");

        const agentDir = path.join(home, "agent-empty");
        const result = await ensureJar4ModelsJson(
          {
            models: { providers: {} },
          },
          agentDir,
  ...
  it("writes models.json for configured providers", async () => {
    await withTempHome(async () => {
      vi.resetModules();
      const { ensureJar4ModelsJson } = await import("./models-config.js");
      const { resolveJar4AgentDir } = await import("./agent-paths.js");

      await ensureJar4ModelsJson(MODELS_CONFIG);

      const modelPath = path.join(resolveJar4AgentDir(), "models.json");
