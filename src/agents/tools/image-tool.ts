...
function resolveDefaultModelRef(cfg?: Jar4Config): {
  provider: string;
  model: string;
} {
  if (cfg) {
...
function pickMaxBytes(cfg?: Jar4Config, maxBytesMb?: number): number | undefined {
...
async function runImagePrompt(params: {
  ...
  const effectiveCfg: Jar4Config | undefined = params.cfg
    ? {
        ...params.cfg,
        agents: {
          ...params.cfg.agents,
  ...
  await ensureJar4ModelsJson(effectiveCfg, params.agentDir);
