...
import { createJar4Tools } from "./jar4-tools.js";
import { __testing, createJar4CodingTools } from "./pi-tools.js";
...
describe("createJar4CodingTools", () => {
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createJar4CodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "jar4-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "jar4-outside.txt");
