...
const makeConfig = (opts?: { fallbacks?: string[]; apiKey?: string }): Jar4Config =>
  ({
    agents: {
      defaults: {
        model: {
...
describe("runEmbeddedPiAgent auth profile rotation", () => {
  it("rotates for auto-pinned profiles", async () => {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "jar4-agent-"));
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "jar4-workspace-"));
    try {
