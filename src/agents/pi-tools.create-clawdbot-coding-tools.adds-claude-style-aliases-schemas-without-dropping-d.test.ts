...
const defaultTools = createJar4CodingTools();

describe("createJar4CodingTools", () => {
  it("keeps read tool image metadata intact", async () => {
    const readTool = defaultTools.find((tool) => tool.name === "read");
    expect(readTool).toBeDefined();

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "jar4-read-"));
    try {
  ...
  it("returns text content without image blocks for text files", async () => {
    const tools = createJar4CodingTools();
    ...
    try {
      ...
      const contents = "Hello from jar4 read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandbox = {
      ...
      workspaceDir: path.join(os.tmpdir(), "jar4-sandbox"),
      agentWorkspaceDir: path.join(os.tmpdir(), "jar4-workspace"),
    ...
    const tools = createJar4CodingTools({ sandbox });
