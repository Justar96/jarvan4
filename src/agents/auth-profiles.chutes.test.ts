...
describe("auth-profiles (chutes)", () => {
  ...
  afterEach(async () => {
    ...
    if (previousAgentDir === undefined) delete process.env.JAR4_AGENT_DIR;
    else process.env.JAR4_AGENT_DIR = previousAgentDir;
  ...
  it("refreshes expired Chutes OAuth credentials", async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "jar4-chutes-"));
    process.env.JAR4_STATE_DIR = tempDir;
    process.env.JAR4_AGENT_DIR = path.join(tempDir, "agents", "main", "agent");
