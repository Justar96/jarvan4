...
function resolveMainSessionKeyForSandbox(params: { cfg?: Jar4Config; agentId: string }): string {
...
export function resolveSandboxRuntimeStatus(params: { cfg?: Jar4Config; sessionKey?: string }): {
  agentId: string;
  sessionKey: string;
...
export function formatSandboxToolPolicyBlockedMessage(params: {
  ...
  lines.push(
    `- See: ${formatCliCommand(`jar4 sandbox explain --session ${runtime.sessionKey}`)}`,
  );
