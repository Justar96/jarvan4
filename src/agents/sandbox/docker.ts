...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "jar4.sandbox=1");
  args.push("--label", `jar4.sessionKey=${params.scopeKey}`);
  args.push("--label", `jar4.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `jar4.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  const result = await execDocker(
    ["inspect", "-f", '{{ index .Config.Labels "jar4.configHash" }}', containerName],
    { allowFailure: true },
  );
  if (result.code !== 0) return null;
...
function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`jar4 sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`jar4 sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("jar4 sandbox recreate --all");
}
