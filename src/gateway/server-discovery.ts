...
export function formatBonjourInstanceName(displayName: string) {
  ...
  if (!trimmed) return "Jar4";
  if (/jar4/i.test(trimmed)) return trimmed;
  return `${trimmed} (Jar4)`;
}

export function resolveBonjourCliPath(opts: ResolveBonjourCliPathOptions = {}): string | undefined {
  const env = opts.env ?? process.env;
  const envPath = env.JAR4_CLI_PATH?.trim();
  ...
  const siblingCli = path.join(execDir, "jar4");
  ...
  const binCli = path.join(cwd, "bin", "jar4.js");
...
export async function resolveTailnetDnsHint(opts?: {
  ...
  const env = opts?.env ?? process.env;
  const envRaw = env.JAR4_TAILNET_DNS?.trim();
