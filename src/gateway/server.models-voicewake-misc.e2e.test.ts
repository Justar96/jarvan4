...
describe("gateway server models + voicewake", () => {
  const setTempHome = (homeDir: string) => {
    ...
    process.env.JAR4_STATE_DIR = path.join(homeDir, ".jar4");
    ...
    return () => {
      ...
      if (prevStateDir === undefined) {
        ...
        process.env.JAR4_STATE_DIR = prevStateDir;
      }
  ...
  test(
    ...
    { timeout: 60_000 },
    async () => {
      const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "jar4-home-"));
      ...
      const onDisk = JSON.parse(
        await fs.readFile(path.join(homeDir, ".jar4", "settings", "voicewake.json"), "utf8"),
...
describe("gateway server misc", () => {
  test("hello-ok advertises the gateway port for canvas host", async () => {
    ...
    const prevCanvasPort = process.env.JAR4_CANVAS_HOST_PORT;
    process.env.JAR4_GATEWAY_TOKEN = "secret";
    testTailnetIPv4.value = "100.64.0.1";
    testState.gatewayBind = "lan";
    const canvasPort = await getFreePort();
    testState.canvasHostPort = canvasPort;
    process.env.JAR4_CANVAS_HOST_PORT = String(canvasPort);
    ...
    if (prevToken === undefined) {
      ...
      process.env.JAR4_GATEWAY_TOKEN = prevToken;
    }
    if (prevCanvasPort === undefined) {
      delete process.env.JAR4_CANVAS_HOST_PORT;
    } else {
      process.env.JAR4_CANVAS_HOST_PORT = prevCanvasPort;
    }
  });
  ...
  test("auto-enables configured channel plugins on startup", async () => {
    const configPath = process.env.JAR4_CONFIG_PATH;
    if (!configPath) throw new Error("Missing JAR4_CONFIG_PATH");
