...
const testConfigRoot = {
  value: path.join(os.tmpdir(), `jar4-gateway-test-${process.pid}-${crypto.randomUUID()}`),
};

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.JAR4_CONFIG_PATH = path.join(root, "jar4.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "jar4.json");
