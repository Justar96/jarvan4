...
import { resolveJar4AgentDir } from "../agents/agent-paths.js";
...
import { ensureJar4ModelsJson } from "../agents/models-config.js";
import { loadConfig } from "../config/config.js";
import type { Jar4Config, ModelProviderConfig } from "../config/types.js";
...
const GATEWAY_LIVE = isTruthyEnvValue(process.env.JAR4_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.JAR4_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.JAR4_LIVE_GATEWAY_PROVIDERS);
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<Jar4Config["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    ...
    agentDir: process.env.JAR4_AGENT_DIR,
    piAgentDir: process.env.PI_CODING_AGENT_DIR,
    stateDir: process.env.JAR4_STATE_DIR,
  };
  ...
  const hostAgentDir = resolveJar4AgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "jar4-live-state-"));
  ...
  process.env.JAR4_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.jar4-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveJar4AgentDir();
  const sanitizedCfg: Jar4Config = {
    ...params.cfg,
  ...
  try {
        ...
        try {
          ...
          const toolProbe = await client.request<AgentFinalPayload>(
            "agent",
            {
              ...
              message:
                "Jar4 live tool probe (local, safe): " +
    ...
    process.env.JAR4_CONFIG_PATH = previous.configPath;
    process.env.JAR4_GATEWAY_TOKEN = previous.token;
    process.env.JAR4_SKIP_CHANNELS = previous.skipChannels;
    process.env.JAR4_SKIP_GMAIL_WATCHER = previous.skipGmail;
    process.env.JAR4_SKIP_CRON = previous.skipCron;
    process.env.JAR4_SKIP_CANVAS_HOST = previous.skipCanvas;
    process.env.JAR4_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.JAR4_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureJar4ModelsJson(cfg);
      ...
      const rawModels = process.env.JAR4_LIVE_GATEWAY_MODELS?.trim();
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.jar4-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
