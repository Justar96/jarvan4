...
describe("gateway e2e", () => {
  it(
    ...
    async () => {
      const prev = {
        ...
        skipBrowser: process.env.JAR4_SKIP_BROWSER_CONTROL_SERVER,
      };

      const { baseUrl: openaiBaseUrl, restore } = installOpenAiResponsesMock();

      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "jar4-gw-mock-home-"));
      ...
      const toolProbePath = path.join(workspaceDir, `.jar4-tool-probe.${nonceA}.txt`);
      await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

      const configDir = path.join(tempHome, ".jar4");
      await fs.mkdir(configDir, { recursive: true });
      const configPath = path.join(configDir, "jar4.json");
      ...
      process.env.JAR4_CONFIG_PATH = configPath;
      ...
      try {
        ...
        process.env.JAR4_CONFIG_PATH = prev.configPath;
        process.env.JAR4_GATEWAY_TOKEN = prev.token;
        process.env.JAR4_SKIP_CHANNELS = prev.skipChannels;
        process.env.JAR4_SKIP_GMAIL_WATCHER = prev.skipGmail;
        process.env.JAR4_SKIP_CRON = prev.skipCron;
        process.env.JAR4_SKIP_CANVAS_HOST = prev.skipCanvas;
        process.env.JAR4_SKIP_BROWSER_CONTROL_SERVER = prev.skipBrowser;
      }
    },
  );
  ...
  it("runs wizard over ws and writes auth token config", { timeout: 90_000 }, async () => {
    ...
    const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "jar4-wizard-home-"));
    process.env.HOME = tempHome;
    ...
    try {
      ...
      await fs.rm(tempHome, { recursive: true, force: true });
      process.env.HOME = prev.home;
      process.env.JAR4_STATE_DIR = prev.stateDir;
      process.env.JAR4_CONFIG_PATH = prev.configPath;
      process.env.JAR4_GATEWAY_TOKEN = prev.token;
      process.env.JAR4_SKIP_CHANNELS = prev.skipChannels;
      process.env.JAR4_SKIP_GMAIL_WATCHER = prev.skipGmail;
