...
import { createJar4Tools } from "../agents/jar4-tools.js";
...
beforeAll(async () => {
  prevGatewayPort = process.env.JAR4_GATEWAY_PORT;
  prevGatewayToken = process.env.JAR4_GATEWAY_TOKEN;
  gatewayPort = await getFreePort();
  process.env.JAR4_GATEWAY_PORT = String(gatewayPort);
  process.env.JAR4_GATEWAY_TOKEN = "test-token";
...
afterAll(async () => {
  ...
  if (prevGatewayPort === undefined) {
    ...
    process.env.JAR4_GATEWAY_PORT = prevGatewayPort;
  }
  if (prevGatewayToken === undefined) {
    delete process.env.JAR4_GATEWAY_TOKEN;
  } else {
    process.env.JAR4_GATEWAY_TOKEN = prevGatewayToken;
  }
});
...
describe("sessions_send gateway loopback", () => {
  it("returns reply when lifecycle ends before agent.wait", async () => {
    ...
    const tool = createJar4Tools().find((candidate) => candidate.name === "sessions_send");
