...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(getHeader(req, "x-jar4-message-channel") ?? "");
  const accountId = getHeader(req, "x-jar4-account-id")?.trim() || undefined;
  ...
  const allTools = createJar4Tools({
