...
export function listWhatsAppAuthDirs(cfg: Jar4Config): string[] {
...
export function hasAnyWhatsAppAuth(cfg: Jar4Config): boolean {
  return listWhatsAppAuthDirs(cfg).some((authDir) => hasWebCredsSync(authDir));
}

export function listWhatsAppAccountIds(cfg: Jar4Config): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultWhatsAppAccountId(cfg: Jar4Config): string {
...
export function resolveWhatsAppAuthDir(params: { cfg: Jar4Config; accountId: string }): {
  authDir: string;
  isLegacy: boolean;
} {
...
export function listEnabledWhatsAppAccounts(cfg: Jar4Config): ResolvedWhatsAppAccount[] {
