...
This directory contains hooks that ship with Jar4. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
metadata:
  { "jar4": { "emoji": "🔗", "events": ["command:new"], "requires": { "bins": ["node"] } } }
---
# Hook Title
...
Hooks can be configured in `~/.jar4/jar4.json`:
...
2. Restart gateway: `pkill -9 -f 'jar4.*gateway' && pnpm jar4 gateway`
3. Enable the hook: `jar4 hooks enable my-hook`
