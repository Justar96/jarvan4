...
2. **Appends to log file** - Writes a JSON line to `~/.jar4/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.jar4/logs/commands.log ~/.jar4/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/jar4`:
   ```
   /home/username/.jar4/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
