...
async function makeTempDir() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "jar4-hooks-e2e-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("hooks install (e2e)", () => {
  ...
  beforeEach(async () => {
    ...
    prevStateDir = process.env.JAR4_STATE_DIR;
    prevBundledDir = process.env.JAR4_BUNDLED_HOOKS_DIR;
    process.env.JAR4_STATE_DIR = path.join(baseDir, "state");
    process.env.JAR4_BUNDLED_HOOKS_DIR = path.join(baseDir, "bundled-none");
    vi.resetModules();
  });
  ...
  afterEach(async () => {
    ...
    if (prevBundledDir === undefined) {
      delete process.env.JAR4_BUNDLED_HOOKS_DIR;
    } else {
      process.env.JAR4_BUNDLED_HOOKS_DIR = prevBundledDir;
    }
  ...
  it("installs a hook pack and triggers the handler", async () => {
    ...
    await fs.writeFile(
      ...
      [
        ...
        'metadata: {"jar4":{"events":["command:new"]}}',
        "---",
        "",
        "# Hello Hook",
        "",
