...
`;
    ...
    expect(parsed.jar4.emoji).toBe("💾");
    expect(parsed.jar4.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata as string);
    expect(parsed.jar4.emoji).toBe("📝");
    expect(parsed.jar4.events).toEqual(["command"]);
    expect(parsed.jar4.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.jar4.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"jar4": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"jar4": {"events": ["test"]}}');
  });
...
describe("resolveJar4Metadata", () => {
  it("extracts jar4 metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveJar4Metadata(frontmatter);
  ...
  it("returns undefined when jar4 key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        jar4: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with Jar4" },
            { id: "npm", kind: "npm", package: "@jar4/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@jar4/hook");
  });
...
`;
    ...
    const jar4 = resolveJar4Metadata(frontmatter);
    expect(jar4).toBeDefined();
    expect(jar4?.emoji).toBe("💾");
    expect(jar4?.events).toEqual(["command:new"]);
    expect(jar4?.requires?.config).toEqual(["workspace.dir"]);
    expect(jar4?.install?.[0].kind).toBe("bundled");
  });
