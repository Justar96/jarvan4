...
import { Jar4App } from "./app";
import "../styles.css";

const originalConnect = Jar4App.prototype.connect;

function mountApp(pathname: string) {
  window.history.replaceState({}, "", pathname);
  const app = document.createElement("jar4-app") as Jar4App;
...
beforeEach(() => {
  Jar4App.prototype.connect = () => {
    // no-op: avoid real gateway WS connections in browser tests
  };
  window.__JAR4_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});

afterEach(() => {
  Jar4App.prototype.connect = originalConnect;
  window.__JAR4_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});
...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/jar4/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/jar4");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/jar4/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__JAR4_CONTROL_UI_BASE_PATH__ = "/jar4";
    const app = mountApp("/jar4/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/jar4");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/jar4/sessions");
  });
